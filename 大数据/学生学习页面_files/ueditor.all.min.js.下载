(function() {
	...
	var browser = UE.browser = function() {
		...
		if (browser.gecko) {
			...
			if (geckoRelease) {
				...
				version = geckoRelease[0] * 10000 + (geckoRelease[1] || 0)
						* 100 + (geckoRelease[2] || 0) * 1;
			}
		}
	...
	var utils = UE.utils = {
		...
		inherits : function(subClass, superClass) {
			var oldP = subClass.prototype, newP = utils
					.makeInstance(superClass.prototype);
		...
		unhtml : function(str, reg) {
			return str ? str.replace(
					reg || /[&<">'](?:(amp|lt|quot|gt|#39|nbsp);)?/g, function(
							a, b) {
						if (b) {
							return a;
						} else {
		...
		fixColor : function(name, value) {
			if (/color/i.test(name) && /rgba?/.test(value)) {
				...
				for (var i = 0, color; color = array[i++];) {
					color = parseInt(color.replace(/[^\d]/gi, ''), 10)
							.toString(16);
		...
		optCss : function(val) {
			...
			return val.replace(/^[ \n\r\t;]*|[ \n\r\t]*$/, '').replace(
					/;([ \n\r\t]+)|\1;/g, ';').replace(
					/(&((l|g)t|quot|#39))?;{2,}/g, function(a, b) {
						return b ? b + ";;" : ';'
					});
		},
	...
	utils.each([ 'String', 'Function', 'Array', 'Number', 'RegExp', 'Object' ],
			function(v) {
				UE.utils['is' + v] = function(obj) {
					return Object.prototype.toString.apply(obj) == '[object '
							+ v + ']';
				}
			});
	...
	var domUtils = dom.domUtils = {
		...
		findParentByTagName : function(node, tagNames, includeSelf, excludeFn) {
			tagNames = utils.listToMap(utils.isArray(tagNames) ? tagNames
					: [ tagNames ]);
			return domUtils.findParent(node, function(node) {
				return tagNames[node.tagName]
						&& !(excludeFn && excludeFn(node));
			}, includeSelf);
		},
		findParents : function(node, includeSelf, filterFn, closerFirst) {
			var parents = includeSelf
					&& (filterFn && filterFn(node) || !filterFn) ? [ node ]
					: [];
		...
		getNextDomNode : function(node, startFromChild, filterFn, guard) {
			return getDomNode(node, 'firstChild', 'nextSibling',
					startFromChild, filterFn, guard);
		},
		isBookmarkNode : function(node) {
			return node.nodeType == 1 && node.id
					&& /^_baidu_bookmark_/i.test(node.id);
		},
		...
		clearEmptySibling : function(node, ignoreNext, ignorePre) {
			function clear(next, dir) {
				...
				while (next
						...
						&& (domUtils.isEmptyInlineElement(next) || !new RegExp(
								'[^\t\n\r' + domUtils.fillChar + ']')
								.test(next.nodeValue))) {
					tmpNode = next[dir];
			...
			if (k)
				while (k--) {
					...
					if (element.addEventListener) {
						...
						var key = type + handler.toString(), index = utils
								.indexOf(handler._d.els, element);
						if (!handler._d[key] || index == -1) {
							...
							if (!handler._d[key]) {
								handler._d[key] = function(evt) {
									return handler.call(evt.srcElement, evt
											|| window.event);
								};
							}
		...
		isSameStyle : function(nodeA, nodeB) {
			var styleA = nodeA.style.cssText.replace(/( ?; ?)/g, ';').replace(
					/( ?: ?)/g, ':'), styleB = nodeB.style.cssText.replace(
					/( ?; ?)/g, ';').replace(/( ?: ?)/g, ':');
			if (browser.opera) {
				styleA = nodeA.style;
				styleB = nodeB.style;
		...
		isBlockElm : function(node) {
			return node.nodeType == 1
					&& (dtd.$block[node.tagName] || styleBlock[domUtils
		...
		trimWhiteTextNode : function(node) {
			function remove(dir) {
				var child;
				while ((child = node[dir]) && child.nodeType == 3
		...
		mergeChild : function(node, tagName, attrs) {
			var list = domUtils.getElementsByTagName(node, node.tagName
					.toLowerCase());
				...
				if (ci.tagName.toLowerCase() == 'span') {
					if (node === ci.parentNode) {
						...
						if (node.childNodes.length == 1) {
							node.style.cssText = ci.style.cssText + ";"
									+ node.style.cssText;
							domUtils.remove(ci, true);
							continue;
						}
					}
					ci.style.cssText = node.style.cssText + ';'
							+ ci.style.cssText;
					if (attrs) {
						...
						if (style) {
							...
							for (var j = 0, s; s = style[j++];) {
								ci.style[utils
										.cssStyleToDomStyle(s.split(':')[0])] = s
										.split(':')[1];
							}
						}
					}
			...
			while (parent && dtd.$removeEmpty[parent.tagName]) {
				if (parent.tagName == node.tagName || parent.tagName == 'A') {
					...
					if (parent.tagName == 'SPAN'
							&& !domUtils.isSameStyle(parent, node)
							|| (parent.tagName == 'A' && node.tagName == 'SPAN')) {
						if (parent.childNodes.length > 1
								|| parent !== node.parentNode) {
							node.style.cssText = parent.style.cssText + ";"
									+ node.style.cssText;
		...
		mergeSibling : function(node, ignorePre, ignoreNext) {
			function merge(rtl, start, node) {
				var next;
				if ((next = node[rtl]) && !domUtils.isBookmarkNode(next)
						&& next.nodeType == 1
		...
		removeAttributes : function(node, attrNames) {
			attrNames = utils.isArray(attrNames) ? attrNames : utils.trim(
					attrNames).replace(/[ ]{2,}/g, ' ').split(' ');
		...
		getComputedStyle : function(element, styleName) {
			...
			if (browser.ie && browser.version < 9 && styleName == 'font-size'
					&& !element.style.fontSize && !dtd.$empty[element.tagName]
			...
			try {
				var value = domUtils.getStyle(element, styleName)
						|| (window.getComputedStyle ? domUtils.getWindow(
								element).getComputedStyle(element, '')
								.getPropertyValue(styleName)
								: (element.currentStyle || element.style)[utils
										.cssStyleToDomStyle(styleName)]);
			} catch (e) {
				return "";
			}
		...
		removeClasses : function(elm, classNames) {
			classNames = utils.isArray(classNames) ? classNames : utils.trim(
					classNames).replace(/[ ]{2,}/g, ' ').split(' ');
		...
		removeStyle : function(element, name) {
			if (browser.ie) {
				...
				if (element.style.removeProperty) {
					...
					element.style.removeAttribute(utils
		...
		scrollToView : function(node, win, offsetTop) {
			var getViewPaneSize = function() {
				...
				return {
					width : (mode ? doc.documentElement.clientWidth
							: doc.body.clientWidth) || 0,
					height : (mode ? doc.documentElement.clientHeight
		...
		isFillChar : function(node, isInStart) {
			return node.nodeType == 3
					&& !node.nodeValue.replace(new RegExp(
							(isInStart ? '^' : '') + domUtils.fillChar), '').length
		},
		...
		fillNode : function(doc, node) {
			var tmpNode = browser.ie ? doc.createTextNode(domUtils.fillChar)
		...
		hasNoAttributes : function(node) {
			return browser.ie ? /^<\w+\s*?>/.test(node.outerHTML)
		...
		isTagNode : function(node, tagName) {
			return node.nodeType == 1
					&& new RegExp('^' + node.tagName + '$', 'i').test(tagName)
		},
		filterNodeList : function(nodelist, filter, forAll) {
			...
			if (!utils.isFunction(filter)) {
				var str = filter;
				filter = function(n) {
					return utils.indexOf(utils.isArray(str) ? str : str
							.split(' '), n.tagName.toLowerCase()) != -1
				};
			}
		...
		isInNodeEndBoundary : function(rng, node) {
			...
			if (start.nodeType == 3
					&& rng.startOffset != start.nodeValue.length) {
				return 0;
			}
			if (start.nodeType == 1
					&& rng.startOffset != start.childNodes.length) {
				return 0;
			}
			while (start !== node) {
	...
	(function() {
		...
		function selectOneNode(rng) {
			return !rng.collapsed && rng.startContainer.nodeType == 1
		...
		function execContentsAction(range, action) {
			var start = range.startContainer, end = range.endContainer, startOffset = range.startOffset, endOffset = range.endOffset, doc = range.document, frag = doc
					.createDocumentFragment(), tmpStart, tmpEnd;
			if (start.nodeType == 1) {
			...
			if (start === end && start.nodeType == 3) {
				frag.appendChild(doc.createTextNode(start.substringData(
			...
			var current, currentLevel, clone = frag, startParents = domUtils
					.findParents(start, true), endParents = domUtils
					.findParents(end, true);
			...
			for (var j = i, si; si = startParents[j]; j++) {
				...
				if (si == start) {
					if (!tmpStart) {
						if (range.startContainer.nodeType == 3) {
							clone.appendChild(doc
									.createTextNode(start.nodeValue
											.slice(startOffset)));
							if (action) {
								start.deleteData(startOffset,
										start.nodeValue.length - startOffset);
							}
						} else {
							clone.appendChild(!action ? start.cloneNode(true)
									: start);
						}
					}
				} else {
				...
				while (current) {
					...
					clone.appendChild(!action ? current.cloneNode(true)
							: current);
					current = si;
				}
				clone = currentLevel;
			...
			for (var j = i, ei; ei = endParents[j]; j++) {
				...
				if (ei == end) {
					if (!tmpEnd && range.endContainer.nodeType == 3) {
						clone.appendChild(doc.createTextNode(end.substringData(
								0, endOffset)));
						if (action) {
				...
				if (j != i || !startParents[i]) {
					while (current) {
						...
						clone.insertBefore(!action ? current.cloneNode(true)
								: current, clone.firstChild);
						current = ei;
					}
				}
				clone = currentLevel;
			...
			if (action) {
				range.setStartBefore(
						!endParents[i] ? endParents[i - 1]
								: !startParents[i] ? startParents[i - 1]
					...
					if (!fillData.nodeValue.replace(fillCharReg, '').length) {
						...
						while (tmpNode
								...
								&& (browser.safari ? !(domUtils.getPosition(
										tmpNode, excludeNode) & domUtils.POSITION_CONTAINS)
						...
						fillData.nodeValue = fillData.nodeValue.replace(
								fillCharReg, '');
					}
				}
			} catch (e) {
		...
		Range.prototype = {
			...
			setStartAfter : function(node) {
				return this.setStart(node.parentNode, domUtils
						.getNodeIndex(node) + 1);
			},
			...
			setEndAfter : function(node) {
				return this.setEnd(node.parentNode,
						domUtils.getNodeIndex(node) + 1);
			},
			setEndBefore : function(node) {
				return this
						.setEnd(node.parentNode, domUtils.getNodeIndex(node));
			},
			...
			setStartAtLast : function(node) {
				return this.setStart(node,
						node.nodeType == 3 ? node.nodeValue.length
			...
			cloneRange : function() {
				var me = this;
				return new Range(me.document).setStart(me.startContainer,
						me.startOffset).setEnd(me.endContainer, me.endOffset);
			},
			...
			shrinkBoundary : function(ignoreEnd) {
				...
				function check(node) {
					return node.nodeType == 1 && !domUtils.isBookmarkNode(node)
			...
			getCommonAncestor : function(includeSelf, ignoreTextNode) {
				...
				if (start === end) {
					...
					return ignoreTextNode && start.nodeType == 3 ? start.parentNode
							: start;
				}
			...
			moveToBookmark : function(bookmark) {
				var start = bookmark.id ? this.document
						.getElementById(bookmark.start) : bookmark.start, end = bookmark.end
						&& bookmark.id ? this.document
						.getElementById(bookmark.end) : bookmark.end;
				this.setStartBefore(start);
			...
			enlarge : function(toBlock, stopFn) {
				var isBody = domUtils.isBody, pre, node, tmp = this.document
						.createTextNode('');
				if (toBlock) {
					...
					while (1) {
						if (domUtils.isBlockElm(node)) {
							node = pre;
							while ((pre = node.previousSibling)
				...
				if (!this.collapsed) {
					while (!domUtils.isBody(this.startContainer)
							&& this.startOffset == this.startContainer[this.startContainer.nodeType == 3 ? 'nodeValue'
									: 'childNodes'].length
							&& this.startContainer[this.startContainer.nodeType == 3 ? 'nodeValue'
									: 'childNodes'].length) {
						this.setStartAfter(this.startContainer);
					}
					while (!domUtils.isBody(this.endContainer)
							&& !this.endOffset
							&& this.endContainer[this.endContainer.nodeType == 3 ? 'nodeValue'
									: 'childNodes'].length) {
			...
			applyInlineStyle : function(tagName, attrs, list) {
				...
				var bookmark = this.createBookmark(), end = bookmark.end, filterFn = function(
						node) {
					return node.nodeType == 1 ? node.tagName.toLowerCase() != 'br'
							: !domUtils.isWhitespace(node);
				}, current = domUtils.getNextDomNode(bookmark.start, false,
						filterFn), node, pre, range = this.cloneRange();
				while (current
					...
					if (current.nodeType == 3 || dtd[tagName][current.tagName]) {
						...
						while (node
								...
								&& node !== end) {
							pre = node;
							node = domUtils.getNextDomNode(node,
									node.nodeType == 1, null, function(parent) {
						...
						if (tagName == 'span'
								...
								&& (aNode = domUtils.findParentByTagName(elm,
										'a', true))) {
						...
						current = domUtils.getNextDomNode(current, true,
								filterFn);
					}
				}
			...
			removeInlineStyle : function(tagNames) {
				...
				while (1) {
					if (start.nodeType == 1) {
						if (utils
								.indexOf(tagNames, start.tagName.toLowerCase()) > -1) {
							break;
						}
				...
				if (start) {
					tmpRange = this.cloneRange().setEndBefore(bookmark.start)
				...
				if (end) {
					tmpRange = this.cloneRange().setStartAfter(bookmark.end)
							.setEndAfter(end);
					frag = tmpRange.extractContents();
					tmpRange.insertNode(frag);
			...
			getClosedNode : function() {
				var node;
				if (!this.collapsed) {
					var range = this.cloneRange().adjustmentBoundary()
							.shrinkBoundary();
			...
			select : browser.ie ? function(noFillData, textRange) {
				...
				if (!this.collapsed) {
					...
					if (!noFillData && this.startContainer.nodeType != 3) {
						var tmpText = this.document.createTextNode(fillChar), tmp = this.document
								.createElement('span');
				...
				return this;
					...
					: function(notInsertFillData) {
						...
						var win = domUtils.getWindow(this.document), sel = win
								.getSelection(), txtNode;
						browser.gecko ? this.document.body.focus() : win
								.focus();
						if (sel) {
							sel.removeAllRanges();
							if (this.collapsed && !notInsertFillData) {
								...
								if (!(start.nodeType == 3 && this.startOffset)
										&& (child ? (!child.previousSibling || child.previousSibling.nodeType != 3)
												: (!start.lastChild || start.lastChild.nodeType != 3))) {
									txtNode = this.document
											.createTextNode(fillChar);
									this.insertNode(txtNode);
									removeFillData(this.document, txtNode);
									mergeSibling(txtNode, 'previousSibling');
									mergeSibling(txtNode, 'nextSibling');
									fillData = txtNode;
									this.setStart(txtNode,
											browser.webkit ? 1 : 0).collapse(
											true);
								}
							}
							var nativeRange = this.document.createRange();
							if (this.collapsed && browser.opera
									&& this.startContainer.nodeType == 1) {
								var child = this.startContainer.childNodes[this.startOffset];
								if (!child) {
									child = this.startContainer.lastChild;
									if (child && domUtils.isBr(child)) {
										this.setStartBefore(child).collapse(
												true);
									}
								} else {
									...
									child
											&& this.setStartBefore(child)
													.collapse(true)
								}
							}
							checkOffset(this);
							nativeRange.setStart(this.startContainer,
									this.startOffset);
							nativeRange.setEnd(this.endContainer,
									this.endOffset);
			...
			inFillChar : function() {
				...
				if (this.collapsed
						&& start.nodeType == 3
						&& start.nodeValue.replace(new RegExp('^'
			...
			createAddress : function(ignoreEnd, ignoreTxt) {
				...
				function getAddress(isStart) {
					...
					var parents = domUtils.findParents(node, true, function(
							node) {
					...
					if (ignoreTxt) {
						if (node.nodeType == 3) {
							...
							firstIndex += (isStart ? me.startOffset
									: me.endOffset)
						} else {
							node = node.childNodes[isStart ? me.startOffset
									: me.endOffset];
							if (node) {
								firstIndex = domUtils.getNodeIndex(node,
										ignoreTxt);
							} else {
						...
						firstIndex = isStart ? domUtils.isFillChar(node) ? 0
								: me.startOffset : me.endOffset
					}
					if (firstIndex < 0) {
						firstIndex = 0;
				...
				if (!ignoreEnd) {
					addr.endAddress = me.collapsed ? []
							.concat(addr.startAddress) : getAddress();
				}
				return addr;
			},
			...
			traversal : function(doFn, filterFn) {
				if (this.collapsed)
					return this;
				var bookmark = this.createBookmark(), end = bookmark.end, current = domUtils
						.getNextDomNode(bookmark.start, false, filterFn);
				while (current
						&& current !== end
						&& (domUtils.getPosition(current, end) & domUtils.POSITION_PRECEDING)) {
					var tmpNode = domUtils.getNextDomNode(current, false,
							filterFn);
					doFn(current);
					current = tmpNode;
				}
	...
	(function() {
		function getBoundaryInformation(range, start) {
			...
			var siblings = parent.children, child, testRange = range
					.duplicate(), startIndex = 0, endIndex = siblings.length - 1, index = -1, distance;
			while (startIndex <= endIndex) {
				index = Math.floor((startIndex + endIndex) / 2);
				child = siblings[index];
				testRange.moveToElementText(child);
				var position = testRange
			...
			while (distance > 0) {
				try {
					var pre = child;
					child = child[position > 0 ? 'previousSibling'
							: 'nextSibling'];
		...
		Selection.prototype = {
			getNative : function() {
				...
				try {
					return !doc ? null : ie && browser.ie < 9 ? doc.selection
			...
			isFocus : function() {
				try {
					return browser.ie && _getIERange(this) || !browser.ie
							&& this.getNative().rangeCount ? true : false;
				} catch (e) {
					return false;
				}
			},
			...
			getStart : function() {
				...
				if (ie) {
					...
					if (start.nodeType == 1 && start.hasChildNodes()) {
						start = start.childNodes[Math.min(
								start.childNodes.length - 1, range.startOffset)];
					}
					if (start.nodeType == 3) {
			...
			getText : function() {
				...
				if (this.isFocus() && (nativeSel = this.getNative())) {
					nativeRange = browser.ie ? nativeSel.createRange()
							: nativeSel.getRangeAt(0);
					return browser.ie ? nativeRange.text : nativeRange
							.toString();
				}
				return '';
			},
	...
	(function() {
		...
		function setValue(form, editor) {
			...
			if (editor.textarea) {
				if (utils.isString(editor.textarea)) {
					for (var i = 0, ti, tis = domUtils.getElementsByTagName(
							form, 'textarea'); ti = tis[i++];) {
						if (ti.id == 'ueditor_textarea_'
			...
			if (!textarea) {
				form.appendChild(textarea = domUtils.createElement(document,
						'textarea', {
							'name' : editor.options.textarea,
							'id' : 'ueditor_textarea_'
									+ editor.options.textarea,
							'style' : "display:none"
						}));
				editor.textarea = textarea;
			}
			textarea.value = editor.hasContents() ? (editor.options.allHtmlEnabled ? editor
					.getAllHtml()
		...
		var Editor = UE.Editor = function(options) {
			...
			EventBase.call(me);
			me.commands = {};
			me.options = utils.extend(utils.clone(options || {}),
					UEDITOR_CONFIG, true);
			me.shortcutkeys = {};
			me.inputRules = [];
			me.outputRules = [];
			me.setOpt({
				...
				iframeCssUrl : me.options.UEDITOR_HOME_URL
						+ 'themes/iframe.css?v=2023-1130-1704',
			...
			if (!utils.isEmptyObject(UE.I18N)) {
				...
				utils.loadFile(document, {
					src : me.options.langPath + me.options.lang + "/"
							+ me.options.lang + ".js?v=2024-0321-1800",
					tag : "script",
		...
		Editor.prototype = {
			...
			render : function(container) {
				var me = this, options = me.options, getStyleValue = function(
						attr) {
				...
				if (container) {
					...
					var html = (ie && browser.version < 9 ? ''
							...
							+ (options.iframeCssUrl ? '<link rel=\'stylesheet\' type=\'text/css\' href=\''
							...
							+ (ie ? 'defer=\'defer\'' : '')
							+ ' id=\'_initialScript\'>'
							+ 'setTimeout(function(){window.parent.UE.instants[\'ueditorInstant'
							+ me.uid
							+ '\']._setup(document);},0);'
											...
											'iframe',
											{
												...
												src : 'javascript:void(function(){document.open();'
														+ (options.customDomain
																&& options.DIYDOMAIN != location.hostname ? 'document.domain="'
																+ options.DIYDOMAIN
																+ '";'
																: '')
														+ 'document.write("'
					...
					setTimeout(function() {
						if (/%$/.test(options.initialFrameWidth)) {
							options.minFrameWidth = options.initialFrameWidth = container.offsetWidth;
							container.style.width = options.initialFrameWidth
									+ 'px';
						}
						if (/%$/.test(options.initialFrameHeight)) {
							options.minFrameHeight = options.initialFrameHeight = container.offsetHeight;
							container.style.height = options.initialFrameHeight
									+ 'px';
						}
					})
				}
			...
			_setup : function(doc) {
				...
				if(typeof(document.body.draggable) != 'undefined' && options.disableDraggable){
					doc.body.draggable = false;
					doc.body.setAttribute("ondragstart","return false");
					doc.body.setAttribute("ondragenter","event.dataTransfer.dropEffect='none'; event.stopPropagation(); event.preventDefault();");
					doc.body.setAttribute("ondragover","event.dataTransfer.dropEffect='none';event.stopPropagation(); event.preventDefault();");
					doc.body.setAttribute("ondrop","event.dataTransfer.dropEffect='none';event.stopPropagation(); event.preventDefault();");
				}
				...
				if (domUtils.isEmptyNode(me.body)) {
					me.body.innerHTML = '<p>' + (browser.ie ? '' : '<br/>')
							+ '</p>';
				}
				if (options.focus) {
					setTimeout(function() {
						me.focus(me.options.focusInEnd);
						!me.options.autoClearinitialContent
								&& me._selectionChange();
					}, 0);
				}
				if (!me.container) {
				...
				try {
					me.document.execCommand('enableInlineTableEditing', false,
							false);
				} catch (e) {
				}
				try {
					me.document.execCommand('enableObjectResizing', false,
							false);
				} catch (e) {
				}
				...
				if (!browser.ie) {
					domUtils.on(me.window, [ 'blur', 'focus' ], function(e) {
						if (e.type == 'blur') {
							...
							try {
								me._bakNativeRange = me.selection.getNative()
										.getRangeAt(0);
				...
				if (browser.gecko && browser.version <= 10902) {
					...
					setInterval(function() {
						me.body.style.height = me.iframe.offsetHeight - 20
								+ 'px'
					}, 100)
				}
			...
			sync : function(formId) {
				var me = this, form = formId ? document.getElementById(formId)
						: domUtils.findParent(me.iframe.parentNode, function(
								node) {
									...
									for ( var i in shortcutkeys) {
										...
										for (var t = 0, ti; ti = tmp[t++];) {
											...
											if (/^(ctrl)(\+shift)?\+(\d+)$/
													...
													|| /^(\d+)$/.test(key)) {
												if (((RegExp.$1 == 'ctrl' ? (e.ctrlKey || e.metaKey)
														: 0)
														&& (RegExp.$2 != "" ? e[RegExp.$2
																.slice(1)
																+ "Key"]
			...
			getAllHtml : function() {
				...
				if (browser.ie && browser.version > 8) {
					...
					utils
							.each(
									me.document.styleSheets,
									function(si) {
										headHtmlForIE9 += (si.href ? '<link rel="stylesheet" type="text/css" href="'
												+ si.href + '" />'
												: '<style>' + si.cssText
														+ '</style>');
									});
				...
				return '<html><head>'
						+ (me.options.charset ? '<meta http-equiv="Content-Type" content="text/html; charset='
								+ me.options.charset + '"/>'
								: '')
						+ (headHtmlForIE9 || me.document
			...
			getPlainTxt : function() {
				var reg = new RegExp(domUtils.fillChar, 'g'), html = this.body.innerHTML
						.replace(/[\n\r]/g, '');
			...
			setContent : function(html, isAppendTo, notFireSelectionchange) {
				...
				html = root.toHtml();
				me.body.innerHTML = (isAppendTo ? me.body.innerHTML : '')
						+ html;
				function isCdataDiv(node) {
					return node.tagName == 'DIV'
							&& node.getAttribute('cdata_tag');
				}
				if (me.options.enterTag == 'p') {
					var child = this.body.firstChild, tmpNode;
					if (!child
							|| child.nodeType == 1
							&& (dtd.$cdata[child.tagName] || isCdataDiv(child) || domUtils
									.isCustomeNode(child))
			...
			focus : function(toEnd) {
				try {
					...
					if (toEnd) {
						rng.setStartAtLast(me.body.lastChild).setCursor(false,
								true);
					} else {
						rng.select(true);
					}
			...
			_initEvents : function() {
				...
				/*domUtils.on(doc, [ 'click', 'contextmenu', 'mousedown',
						'keydown', 'keyup', 'keypress', 'mouseup', 'mouseover',
						'mouseout', 'selectstart' ], me._proxyDomEvent);*/
			...
			_selectionChange : function(delay, evt) {
				...
				var mouseX, mouseY;
				if (browser.ie && browser.version < 9 && evt
						&& evt.type == 'mouseup') {
				...
				_selectionChangeTimer = setTimeout(function() {
					...
					var ieRange;
					if (hackForMouseUp
							&& me.selection.getNative().type == 'None') {
					...
					if (me.selection._cachedRange
							&& me.selection._cachedStartElement) {
			...
			execCommand : function(cmdName) {
				...
				var me = this, result, cmd = me.commands[cmdName]
						|| UE.commands[cmdName];
			...
			hasContents : function(tags) {
				...
				for (i = 0; ci = tags[i++];) {
					var nodes = domUtils
							.getElementsByTagName(this.document, ci);
			...
			setDisabled : function(except) {
				...
				if (me.body.contentEditable == 'true') {
					if (!me.lastBk) {
						me.lastBk = me.selection.getRange()
								.createBookmark(true);
					}
					me.body.contentEditable = false;
			...
			_setDefaultContent : function() {
				function clear() {
					...
					if (me.document.getElementById('initContent')) {
						me.body.innerHTML = '<p>' + (ie ? '' : '<br/>')
								+ '</p>';
						me
								.removeListener('firstBeforeExecCommand focus',
										clear);
						setTimeout(function() {
							me.focus();
							me._selectionChange();
			...
			contentTransPlainTxt: function(html){
				if(!html){
					return html;
				}
				var reg = new RegExp(domUtils.fillChar, 'g'),
			...
			pureWordCount : function(input) {//计算输入的纯字数
				var detectedWords = this.pureWord(input);
				return detectedWords.length;
			},
			pureWord : function(input) {
				var text = '';
				if(input != undefined){
					text = input;
					text = this.contentTransPlainTxt(text);
				}else{
					text = this.getPlainTxt();
				}
				if(text){
				  text = text.replace(/<[^>]+>/g,'');
			    }
				...
				for (var i = 0; i < combinedPunctionations.length; i++) {
					var punctuation = combinedPunctionations[i];
				...
				var reg = new RegExp(common + '[' + cjk + jp + kr + ']', 'g');
	...
	//filterWord 模块 引用的是 1.4.3.3 version，解决word 粘贴空格bug问题  
	...
	(function() {
		...
		function nodeToHtml(node, arr, formatter, current) {
			...
			case 'root':
				for (var i = 0, ci; ci = node.children[i++];) {
					if (formatter && ci.type == 'element'
							&& !dtd.$inlineWithA[ci.tagName] && i > 1) {
		...
		function isText(node, arr) {
			arr.push(node.parentNode.tagName == 'pre' ? node.data : node.data
					.replace(/[ ]{2}/g, ' &nbsp;'))
		}
		function isElement(node, arr, formatter, current) {
			...
			arr.push('<' + node.tagName + (attrhtml ? ' ' + attrhtml : '')
					+ (dtd.$empty[node.tagName] ? '\/' : '') + '>');
		...
		uNode.prototype = {
			...
			setStyle : function(name, val) {
				function exec(name, val) {
					...
					if (val) {
						cssStyle = name + ':' + utils.unhtml(val) + ';'
								+ cssStyle
					}
				}
				var cssStyle = this.getAttr('style');
	...
	var htmlparser = UE.htmlparser = function(htmlstr, ignoreBlank) {
		...
		function element(parent, tagName, htmlattr) {
			...
			if (needParentTag = needParentNode[tagName]) {
				...
				while (tmpParent.type != 'root') {
					if (utils.isArray(needParentTag) ? utils.indexOf(
							needParentTag, tmpParent.tagName) != -1
				...
				if (!hasParent) {
					parent = element(parent,
							utils.isArray(needParentTag) ? needParentTag[0]
									: needParentTag)
				}
			}
			var elm = new uNode({
			...
			if (htmlattr) {
				...
				while (match = re_attr.exec(htmlattr)) {
					attrs[match[1].toLowerCase()] = utils.unhtml(match[2]
							|| match[3] || match[4])
				}
				elm.attrs = attrs;
			}
		...
		while (match = re_tag.exec(htmlstr)) {
			...
			try {
				...
				if (match[3]) {
					currentParent = element(currentParent, match[3]
							.toLowerCase(), match[4]);
				} else if (match[1]) {
			...
			case 'element':
				...
				if (val = rules[node.tagName]) {
					if (val === '-') {
						...
						var parentNode = node.parentNode, index = node
								.getIndex();
						val(node);
						if (node.parentNode) {
							if (node.children) {
							...
							.traversal(function(node) {
								if (node.type == 'element') {
									...
									case 'script':
										node
												.setAttr({
													...
													cdata_data : encodeURIComponent(node
															.innerText()
															|| '')
												});
										node.tagName = 'div';
									...
									case 'div':
										...
										if (p.firstChild()) {
											node.parentNode.insertBefore(p,
													node);
										}
										node.parentNode.removeChild(node);
										break;
									...
									case 'caption':
										if (!node.children
											...
											node
													.appendChild(browser.ie ? UE.uNode
															.createText(' ')
															: UE.uNode
																	.createElement('br'))
										}
						...
						if (node.type == 'element') {
							...
							case 'div':
								if (val = node.getAttr('cdata_tag')) {
									node.tagName = val;
									node.appendChild(UE.uNode.createText(node
											.getAttr('cdata_data')));
									node.setAttr({
										cdata_tag : '',
										cdata_data : ''
	...
	UE.commands['inserthtml'] = {
		execCommand : function(command, html, notNeedFilter) {
			...
			if (!range.collapsed) {
				...
				if (range.startOffset == 0) {
					...
					if (domUtils.isBoundaryNode(tmpNode, 'firstChild')) {
						tmpNode = range.endContainer;
						if (range.endOffset == (tmpNode.nodeType == 3 ? tmpNode.nodeValue.length
								: tmpNode.childNodes.length)
								&& domUtils
										.isBoundaryNode(tmpNode, 'lastChild')) {
							me.body.innerHTML = '<p>'
									+ (browser.ie ? '' : '<br/>') + '</p>';
							range.setStart(me.body.firstChild, 0)
									.collapse(true)
						}
					}
				}
			...
			var li = domUtils.findParentByTagName(range.startContainer, 'li',
					true);
			if (li) {
				var next, last;
				while (child = div.firstChild) {
					while (child
							&& (child.nodeType == 3
									|| !domUtils.isBlockElm(child) || child.tagName == 'HR')) {
				...
				li = domUtils.findParentByTagName(range.startContainer, 'li',
						true);
				...
				while (child = div.firstChild) {
					...
					if (!hadBreak && child.nodeType == domUtils.NODE_ELEMENT
						...
						if (parent
							...
							if (!browser.ie && (next = child.nextSibling)
									&& domUtils.isBlockElm(next)
									&& next.lastChild
									&& !domUtils.isBr(next.lastChild)) {
								next.appendChild(me.document
										.createElement('br'));
							}
							hadBreak = 1;
						}
				...
				if (domUtils.isBlockElm(child) && domUtils.isEmptyNode(child)) {
					if (nextNode = child.nextSibling) {
						...
						if (nextNode.nodeType == 1
								&& dtd.$block[nextNode.tagName]) {
							range.setStart(nextNode, 0).collapse(true)
									.shrinkBoundary()
						}
					} else {
						try {
			...
			setTimeout(
					function() {
						range = me.selection.getRange();
						range.scrollToView(me.autoHeightEnabled,
								me.autoHeightEnabled ? domUtils
										.getXY(me.iframe).y : 0);
	...
	UE.plugins['autotypeset'] = function() {
		...
		function isLine(node, notEmpty) {
			...
			if (node && node.parentNode && tags[node.tagName.toLowerCase()]) {
				...
				return notEmpty ? !domUtils.isEmptyBlock(node) : domUtils
						.isEmptyBlock(node, new RegExp('[\\s'
								+ domUtils.fillChar + ']', 'g'));
			}
		}
			...
			for (var i = 0, ci; ci = nodes[i++];) {
				...
				if (isLine(ci)) {
					if (opt.mergeEmptyline) {
						var next = ci.nextSibling, tmpNode, isBr = domUtils
								.isBr(ci);
						while (isLine(next)) {
							tmpNode = next;
					...
					if (opt.removeEmptyline && domUtils.inDoc(ci, cont)
				...
				if (opt.removeClass && ci.className
				...
				if (opt.imageBlockLine && ci.tagName.toLowerCase() == 'img'
					...
					if (html) {
						...
						case 'none':
							...
							tmpNode = pN;
							if (tmpNode.tagName == 'P'
									&& domUtils.getStyle(tmpNode, 'text-align') == 'center') {
								if (!domUtils.isBody(tmpNode)
										&& domUtils
												.getChildCount(
														tmpNode,
														function(node) {
						...
						case 'center':
							if (me.queryCommandValue('imagefloat') != 'center') {
								...
								tmpNode = img;
								while (pN
										&& domUtils.getChildCount(pN, function(
												node) {
											return !domUtils.isBr(node)
													&& !domUtils
															.isWhitespace(node)
	...
	/*UE.plugins['autosubmit'] = function() {
		...
		me.commands['autosubmit'] = {
			execCommand : function() {
				var me = this, form = domUtils.findParentByTagName(me.iframe,
						"form", false);
				if (form) {
	...
	UE.plugins['background'] = function() {
		...
		me.addListener("getAllHtml", function(type, headHtml) {
			var body = this.body, su = domUtils.getComputedStyle(body,
					"background-image"), url = "";
			if (su.indexOf(me.options.imagePath) > 0) {
				url = su.substring(su.indexOf(me.options.imagePath),
						su.length - 1).replace(/"|\(|\)/ig, "");
			} else {
				url = su != "none" ? su.replace(/url\("?|"?\)/ig, "") : "";
			}
			var html = '<style type="text/css">body{';
			var bgObj = {
				"background-color" : domUtils.getComputedStyle(body,
						"background-color")
						|| "#ffffff",
				'background-image' : url ? 'url(' + url + ')' : '',
				'background-repeat' : domUtils.getComputedStyle(body,
						"background-repeat")
						|| "",
				'background-position' : browser.ie ? (domUtils
						.getComputedStyle(body, "background-position-x")
						+ " " + domUtils.getComputedStyle(body,
	...
	UE.commands['imagefloat'] = {
						...
						if (tmpNode.tagName == 'P'
							...
							if (!domUtils.isBody(tmpNode)
								...
								if (pre && next && pre.nodeType == 1
									...
									domUtils
											.setStyle(tmpNode, 'text-align', '');
								}
							}
							range.selectNode(img).select();
						}
						domUtils.setStyle(img, 'float', align == 'none' ? ''
								: align);
						if (align == 'none') {
						...
						if (me.queryCommandValue('imagefloat') != 'center') {
							...
							me.execCommand('insertHtml',
									'<p id="_img_parent_tmp" style="text-align:center">'
											+ pN.innerHTML + '</p>');
							tmpNode = me.document
									.getElementById('_img_parent_tmp');
		...
		queryCommandValue : function() {
			...
			if (startNode && startNode.nodeType == 1
				...
				floatStyle = startNode.getAttribute('align')
						|| domUtils.getComputedStyle(startNode, 'float');
				if (floatStyle == 'none') {
					floatStyle = domUtils.getComputedStyle(
							startNode.parentNode, 'text-align') == 'center' ? 'center'
							: floatStyle;
				}
				return {
					left : 1,
	...
	UE.commands['insertimage'] = {
		execCommand : function(cmd, opt) {
			...
			var me = this, range = me.selection.getRange(), img = range
					.getClosedNode();
			if (img && /img/i.test(img.tagName)
				...
				if (opt.length == 1) {
					str = '<img src="'
							...
							+ (ci['floatStyle'] == 'left'
									|| ci['floatStyle'] == 'right' ? ' style="float:'
									+ ci['floatStyle'] + ';"'
									: '')
							+ (ci.title && ci.title != "" ? ' title="'
									+ ci.title + '"' : '')
							+ (ci.border && ci.border != "0" ? ' border="'
									+ ci.border + '"' : '')
							+ (ci.alt && ci.alt != "" ? ' alt="' + ci.alt + '"'
									: '')
							+ (ci.hspace && ci.hspace != "0" ? ' hspace = "'
									+ ci.hspace + '"' : '')
							+ (ci.vspace && ci.vspace != "0" ? ' vspace = "'
									+ ci.vspace + '"' : '') + '/>';
	...
	UE.commands['review'] = {
		execCommand : function(cmd) {
			...
			if (img && /img/i.test(img.tagName)
				...
				if (path.indexOf("p.ananas.chaoxing.com") < 0) {
					return;
				}
				var arr = path.split("star3");
				if(arr.length > 0){
					path = location.protocol + "//" + location.hostname + "/p-ans/star3" + arr[1];
				}
				if (path.indexOf("origin") != -1) {
					path = path.replace("origin", "750_1024");
				}
				if (path.indexOf("375_1024") != -1) {
					path = path.replace("375_1024", "750_1024");
				}
				imageEditor.loadImageFromURL(path, 'SampleImage').then(function(sizeValue) {
	...
	UE.plugins['justify'] = function() {
		var me = this, block = domUtils.isBlockElm, defaultValue = {
			...
			var bookmark = range.createBookmark(), filterFn = function(node) {
				return node.nodeType == 1 ? node.tagName.toLowerCase() != 'br'
						&& !domUtils.isBookmarkNode(node) : !domUtils
						.isWhitespace(node);
			};
			range.enlarge(true);
			var bookmark2 = range.createBookmark(), current = domUtils
					.getNextDomNode(bookmark2.start, false, filterFn), tmpRange = range
					.cloneRange(), tmpNode;
			while (current
					&& !(domUtils.getPosition(current, bookmark2.end) & domUtils.POSITION_FOLLOWING)) {
				if (current.nodeType == 3 || !block(current)) {
					tmpRange.setStartBefore(current);
					while (current && current !== bookmark2.end
							&& !block(current)) {
						tmpNode = current;
		...
		UE.commands['justify'] = {
			...
			queryCommandValue : function() {
				var startNode = this.selection.getStart(), value = domUtils
						.getComputedStyle(startNode, 'text-align');
				return defaultValue[value] ? value : 'left';
			},
			queryCommandState : function() {
				var start = this.selection.getStart(), cell = start
						&& domUtils.findParentByTagName(start, [ "td", "th",
								"caption" ], true);
				return cell ? -1 : 0;
			}
		};
	...
	UE.plugins['font'] = function() {
			...
			while (parent = node.parentNode) {
				if (parent.tagName == 'SPAN'
						&& domUtils.getChildCount(parent, function(child) {
							return !domUtils.isBookmarkNode(child)
									&& !domUtils.isBr(child)
						}) == 1) {
					parent.style.cssText += node.style.cssText;
					domUtils.remove(node, true);
			...
			if (needSetChild[cmdName]) {
				...
				if (!rng.collapsed && rng.startContainer.nodeType == 1) {
					...
					if (start && domUtils.isTagNode(start, 'span')) {
						...
						utils
								.each(
										domUtils.getElementsByTagName(start,
												'span'),
										function(span) {
											if (!span.parentNode
													|| domUtils
											...
											domUtils.removeStyle(span,
													needSetChild[cmdName]);
											if (span.style.cssText.replace(
													/^\s+$/, '').length == 0) {
		...
		function mergesibling(rng, cmdName, value) {
			...
			utils
					.each(
							...
							function(span) {
								if (!span.parentNode
										|| domUtils.isBookmarkNode(span))
									return;
								if (/\s*border\s*:\s*none;?\s*/i
										.test(span.style.cssText)) {
									if (/^\s*border\s*:\s*none;?\s*$/
											.test(span.style.cssText)) {
										domUtils.remove(span, true);
									} else {
								...
								if (/border/i.test(span.style.cssText)
		...
		me.addInputRule(function(root) {
			utils.each(root.getNodesByTagName('u s del font strike'), function(
					node) {
				if (node.tagName == 'font') {
					...
					var val = node.tagName == 'u' ? 'underline'
							: 'line-through';
					node.attrs = {
			...
			(function(cmd, style) {
				UE.commands[cmd] = {
					execCommand : function(cmdName, value) {
						value = value
								|| (this.queryCommandState(cmdName) ? 'none'
						...
						if (value == 'default') {
							...
							if (!range.collapsed) {
								if (needCmd[cmd] && me.queryCommandValue(cmd)) {
									me.execCommand('removeFormat', 'span,a',
											style);
								}
								range = me.selection.getRange();
								...
								var span = domUtils.findParentByTagName(
										range.startContainer, 'span', true);
								text = me.document.createTextNode('font');
								if (span
										&& !span.children.length
										&& !span[browser.ie ? 'innerText'
												: 'textContent'].replace(
												fillCharReg, '').length) {
									...
									if (needCmd[cmd]) {
										...
										me.execCommand('removeFormat',
												'span,a', style, null);
										span = domUtils.findParentByTagName(
												text, 'span', true);
									...
									if (needCmd[cmd]) {
										if (domUtils.findParentByTagName(text,
												'a', true)) {
											range.setStartBefore(text)
													.setCursor();
											domUtils.remove(text);
									...
									if (opera) {
										setTimeout(function() {
											range.setStart(span, 0).collapse(
													true);
											mergesibling(range, cmdName, value);
											range.select();
										});
					...
					queryCommandValue : function(cmdName) {
						...
						if (cmdName == 'underline'
							...
							while (tmpNode && !domUtils.isBlockElm(tmpNode)
									&& !domUtils.isBody(tmpNode)) {
								if (tmpNode.nodeType == 1) {
									value = domUtils.getComputedStyle(tmpNode,
											style);
									if (value != 'none') {
										return value;
									}
						...
						if (cmdName == 'fontborder') {
							...
							while (tmp && dtd.$inline[tmp.tagName]) {
								if (val = domUtils.getComputedStyle(tmp,
										'border')) {
						...
						if (cmdName == 'FontSize') {
							var styleVal = domUtils.getComputedStyle(startNode,
									style), tmp = /^([\d\.]+)(\w+)$/
									.exec(styleVal);
							if (tmp) {
					...
					queryCommandState : function(cmdName) {
						...
						if (cmdName == 'fontborder') {
							...
							return val == (cmdName == 'underline' ? 'underline'
									: 'line-through');
						}
					}
				};
	...
	UE.plugins['link'] = function() {
		...
		UE.commands['unlink'] = {
			execCommand : function() {
				...
				if (range.collapsed
						&& !domUtils.findParentByTagName(range.startContainer,
								'a', true)) {
					return;
				}
			...
			queryCommandState : function() {
				return !this.highlight && this.queryCommandValue('link') ? 0
						: -1;
			}
		};
		function doLink(range, opt, me) {
			var rngClone = range.cloneRange(), link = me
					.queryCommandValue('link');
			...
			if (start.nodeType == 1 && link) {
				...
				if (start
						...
						&& start.tagName == 'A'
						&& /^(?:https?|ftp|file)\s*:\s*\/\//
								.test(start[browser.ie ? 'innerText'
										: 'textContent'])) {
					start[browser.ie ? 'innerText' : 'textContent'] = utils
							.html(opt.textValue || opt.href);
				}
			}
			...
			if (rngClone.collapsed) {
				...
				start = domUtils.findParentByTagName(rngClone.startContainer,
						'a', true);
		...
		UE.commands['link'] = {
			execCommand : function(cmdName, opt) {
				...
				opt.textValue
						&& (opt.textValue = utils.unhtml(opt.textValue,
								/[<">]/g));
			...
			queryCommandValue : function() {
				...
				if (range.collapsed) {
					...
					if (node
							&& (node = domUtils.findParentByTagName(node, 'a',
									true))
					...
					var start = range.startContainer.nodeType == 3
							...
							: range.startContainer.childNodes[range.startOffset], end = range.endContainer.nodeType == 3
							|| range.endOffset == 0 ? range.endContainer
							: range.endContainer.childNodes[range.endOffset - 1], common = range
							.getCommonAncestor();
					node = domUtils.findParentByTagName(common, 'a', true);
					if (!node && common.nodeType == 1) {
						var as = common.getElementsByTagName('a'), ps, pe;
						for (var i = 0, ci; ci = as[i++];) {
							ps = domUtils.getPosition(ci, start), pe = domUtils
									.getPosition(ci, end);
							if ((ps & domUtils.POSITION_FOLLOWING || ps
									& domUtils.POSITION_CONTAINS)
									&& (pe & domUtils.POSITION_PRECEDING || pe
											& domUtils.POSITION_CONTAINS)) {
								node = ci;
								break;
							}
			...
			queryCommandState : function() {
				var img = this.selection.getRange().getClosedNode(), flag = img
						&& (img.className == "edui-faked-video");
				return flag ? -1 : 0;
			}
		};
			...
			execCommand : function(cmdName, tags, style, attrs, notIncludeA) {
				var tagReg = new RegExp('^(?:'
						+ (tags || this.options.removeFormatTags).replace(/,/g,
								'|') + ')$', 'i'), removeFormatAttributes = style ? []
						: (attrs || this.options.removeFormatAttributes)
								.split(','), range = new dom.Range(
						this.document), bookmark, node, parent, filter = function(
						node) {
					return node.nodeType == 1;
				};
	...
	UE.plugins['blockquote'] = function() {
		...
		function getObj(editor) {
			return domUtils.filterNodeList(editor.selection
					.getStartElementPath(), 'blockquote');
		}
		me.commands['blockquote'] = {
			execCommand : function(cmdName, attrs) {
				...
				if (obj) {
					...
					var tmpRange = range.cloneRange(), node = tmpRange.startContainer.nodeType == 1 ? tmpRange.startContainer
					...
					if (doEnd) {
						preNode = node = node = tmpRange.endContainer.nodeType == 1 ? tmpRange.endContainer
					...
					var childs = domUtils.getElementsByTagName(node,
							'blockquote');
	...
	UE.commands['touppercase'] = UE.commands['tolowercase'] = {
		execCommand : function(cmd) {
			...
			var bk = rng.createBookmark(), bkEnd = bk.end, filterFn = function(
					node) {
	...
	UE.commands['indent'] = {
		execCommand : function() {
			var me = this, value = me.queryCommandState("indent") ? "0em"
		...
		queryCommandState : function() {
			var pN = domUtils.filterNodeList(this.selection
					.getStartElementPath(), 'p h1 h2 h3 h4 h5 h6');
			return pN && pN.style.textIndent && parseInt(pN.style.textIndent) ? 1
					: 0;
		}
	};
	UE.commands['print'] = {
	...
	UE.commands['preview'] = {
		execCommand : function() {
			...
			d.open();
			d.write('<html><head><script src="' + this.options.UEDITOR_HOME_URL
					+ 'ueditor.parse.js"></script><script>'
					+ "setTimeout(function(){uParse('div',{"
					+ "    'highlightJsUrl':'" + this.options.UEDITOR_HOME_URL
					+ "third-party/SyntaxHighlighter/shCore.js',"
					+ "    'highlightCssUrl':'" + this.options.UEDITOR_HOME_URL
		...
		me.commands['selectall'] = {
			execCommand : function() {
				...
				if (domUtils.isEmptyBlock(body)) {
					if (browser.opera && body.firstChild
	...
	UE.plugins['paragraph'] = function() {
		var me = this, block = domUtils.isBlockElm, notExchange = [ 'TD', 'LI',
				'PRE' ], doParagraph = function(range, style, attrs,
				sourceCmdName) {
			var bookmark = range.createBookmark(), filterFn = function(node) {
				return node.nodeType == 1 ? node.tagName.toLowerCase() != 'br'
						&& !domUtils.isBookmarkNode(node) : !domUtils
			...
			while (current
				...
				if (current.nodeType == 3 || !block(current)) {
					...
					if (block(parent) && !domUtils.isBody(para.parentNode)
							&& utils.indexOf(notExchange, parent.tagName) == -1) {
						if (!(sourceCmdName && sourceCmdName == 'customstyle')) {
							parent.getAttribute('dir')
									&& para.setAttribute('dir', parent
											.getAttribute('dir'));
							parent.style.cssText
									&& (para.style.cssText = parent.style.cssText
											+ ';' + para.style.cssText);
		...
		me.commands['paragraph'] = {
			execCommand : function(cmdName, style, attrs, sourceCmdName) {
				...
				if (browser.gecko && range.collapsed
			...
			queryCommandValue : function() {
				var node = domUtils.filterNodeList(this.selection
						.getStartElementPath(), 'p h1 h2 h3 h4 h5 h6');
	...
	(function() {
		var block = domUtils.isBlockElm, getObj = function(editor) {
			return domUtils.filterNodeList(editor.selection
					.getStartElementPath(), function(n) {
	...
	UE.plugins['horizontal'] = function() {
		...
		me.commands['horizontal'] = {
			...
			queryCommandState : function() {
				return domUtils.filterNodeList(this.selection
						.getStartElementPath(), 'table') ? -1 : 0;
			}
		};
	...
	UE.plugins['rowspacing'] = function() {
		...
		me.commands['rowspacing'] = {
			...
			queryCommandValue : function(cmdName, dir) {
				var pN = domUtils.filterNodeList(this.selection
						.getStartElementPath(), function(node) {
					return domUtils.isBlockElm(node)
				}), value;
				if (pN) {
					value = domUtils.getComputedStyle(pN, 'margin-' + dir)
							.replace(/[^\d]/g, '');
	...
	UE.plugins['insertcode'] = function() {
		...
		me.commands['insertcode'] = {
			execCommand : function(cmd, lang) {
				var me = this, rng = me.selection.getRange(), pre = domUtils
						.findParentByTagName(rng.startContainer, 'pre', true);
				if (pre) {
						...
						utils
								.each(
										UE
												.filterNode(
														UE
																.htmlparser(div.innerHTML
																		.replace(
																				/[\r\t]/g,
																				'')),
														me.options.filterTxtRules).children,
										function(node) {
											if (browser.ie
													&& browser.version > 8) {
												if (node.type == 'element') {
					...
					if (tmpNode
							&& (tmpNode.nodeType == 3
									...
									&& browser.ie && browser.version == 6 || domUtils
									.isEmptyBlock(tmpNode))) {
						domUtils.remove(tmpNode)
					}
					var rng = me.selection.getRange();
		...
		me.queryCommandState = function(cmd) {
			var me = this;
			if (!me.notNeedCodeQuery[cmd.toLowerCase()] && me.selection
		...
		me
				.addListener(
						...
						function() {
							var rng = me.selection.getRange();
							var pre = domUtils.findParentByTagName(
									rng.startContainer, 'pre', true);
							if (pre) {
								...
								if (!browser.ie) {
									var tmpNode = me.document
											.createElement('br'), pre;
									rng.insertNode(tmpNode).setStartAfter(
									...
									if (pre) {
										...
										if (str) {
											str = me.document
													.createTextNode(str);
											rng.insertNode(str).setStartAfter(
													str);
										}
									}
									...
									if (browser.version > 8) {
										...
										if (rng.startOffset == 0) {
											...
											rng.insertNode(txt).setStartAfter(
													txt);
											var fillchar = me.document
													.createTextNode(' ');
											...
											rng.insertNode(fillchar).setStart(
													fillchar, 0).collapse(true)
													.select(true)
										}
									} else {
										var tmpNode = me.document
										...
										rng
												.insertNode(me.document
														.createTextNode(domUtils.fillChar));
										rng.setStartAfter(tmpNode);
										pre = tmpNode.previousSibling;
										var tmp;
										while (pre) {
		...
		me.addListener('tabkeydown', function(cmd, evt) {
			var rng = me.selection.getRange();
			var pre = domUtils.findParentByTagName(rng.startContainer, 'pre',
					true);
			if (pre) {
				me.fireEvent('saveScene');
				if (evt.shiftKey) {
					...
					if (!rng.collapsed) {
						...
						while (start) {
							if (pre.firstChild === start
									&& !domUtils.isBr(start)) {
								pre.insertBefore(me.document
		...
		me
				.addListener(
						...
						function(evtName, html) {
							var me = this, rng = me.selection.getRange(), pre = domUtils
									.findParentByTagName(rng.startContainer,
											'pre', true);
							if (pre) {
								if (!rng.collapsed) {
									rng.deleteContents()
								...
								if (browser.ie && browser.version > 8) {
									...
									var tmpNode = me.document
											.createTextNode(utils.html(htmlstr
													.replace(/&nbsp;/g, ' ')));
									rng.insertNode(tmpNode).selectNode(tmpNode)
											.select();
								} else {
									var frag = me.document
		...
		me.addListener('keydown', function (cmd, evt) {
			...
			if (range.collapsed) {
				if (keyCode == 8) { //删除
					...
					if (start.nodeType == 1 && range.startOffset == 0 && start.previousSibling && start.previousSibling.nodeType == 1
						&& start.previousSibling.getAttribute('class') &&
		...
		me.addListener('delkeydown', function(type, evt) {
			...
			rng.txtToElmBoundary(true);
			var start = rng.startContainer;
			if (domUtils.isTagNode(start, 'pre') && rng.collapsed
	...
	UE.commands['cleardoc'] = {
		execCommand : function(cmdName) {
			var me = this, enterTag = me.options.enterTag, range = me.selection
					.getRange();
			if (enterTag == "br") {
	...
	UE.plugins['insertcodedefined'] = function () {
		var me = this;
			...
			codeTool.prototype = {
				init: function (editor) {
					...
					resizer.innerHTML = '<div class="codetoolDiv clearfix">' +
						...
						'<input type="text" name="" value="" class="dataSearch_input fl" placeholder="' + '搜索' + '">' +//todo 国际化
						...
						'<div class="btn fl linebreak"><i></i><span>' + '自动换行' + '</span></div>' +
						'<div class="btn fl copy"><i></i><span>' + '复制' + '</span></div>' +
						'<div class="btn fl remove pre"><i></i><span>' + '删除' + '</span></div>' +
						'</div>' +
						'</div>';
				...
				initEvents: function () {
					...
					$(_this.resizer).on('click', '.langList .item', function (e) {
						...
						if (typeof (content) != "undefined" && content.trim() != "" && content != null) {
					...
					$(_this.resizer).on('click','.linebreak',function(){
						if($(_this.target).hasClass('autolinebreak')){
							...
							$(this).find('span').text('自动换行')
						}else{
							$(_this.target).addClass('autolinebreak')
							$(this).find('span').text('取消自动换行')
						}
					})
					//复制
					$(_this.resizer).on('click','.copy',function(){
						if (!window.dt) {
							utils.loadFile(document, {
								src: "/mooc2/js/editor/third-party/clipboard-polyfill.promise.js",
								tag: "script",
								type: "text/javascript",
								defer: "defer"
							}, function () {
					...
					function copyCode(node){
						...
						var html = '<pre class= "'+(node.getAttribute('autolinebreak')||'')+ ' prettyprint linenums prettyprinted' +'"><code class="language-'+lang+'" lang="'+lang+'">' + node.innerText.replace(/\n/g,'<br>') + '</code></pre>';
						dt.setData("text/html", html);
						dt.setData('text/plain', node.innerText)
						clipboard.write(dt).then(function () {
							$('.edui-editor-resizer').removeClass('hover');
							if ($('.toolTipBox').length > 0) {	$('.toolTipBox').remove()}
							$('body').append('<div class="toolTipBox"><i class="popicon"><img style="display: block;" src="/mooc2/images/right.png" /></i><span class="tipstext">' + '复制成功' + '<span></div>')
							setTimeout(function () {
								$('.toolTipBox').fadeOut();
							}, 1500)
						}, function (err) {
					...
					$(_this.resizer).on('click','.remove',function() {
						...
						_this.target.querySelector('code').parentNode.remove();
					})
				},
				show: function (targetObj) {
					...
					if(targetObj.className.indexOf('autolinebreak') > -1){
						_this.resizer.querySelector('.linebreak').querySelector('span').innerText = '取消自动换行'
					}else{
						_this.resizer.querySelector('.linebreak').querySelector('span').innerText = '自动换行'
					}
					var temphtml = '';
					for(var i = 0; i < _this.codeList.length;i++){
						temphtml += '<p class="item">'+_this.codeList[i]+'</p>'
		...
		me.commands['insertcodedefined'] = {
			execCommand: function (cmd, lang) {
				...
				if (pre) {
					...
					var html = '<pre class="line-numbers" ><code id="coder" lang="Plain Text" class="language-plain">' + code + '</code></pre><p></p>'
		...
		me.addListener('click', function (cmd, evt) {
			...
			//点击录音标注
			if (target.className && target.className.indexOf('record-list-tit') > -1) {
	...
	UE.plugins['anchor'] = function() {
		...
		me.commands['anchor'] = {
			execCommand : function(cmd, name) {
				var range = this.selection.getRange(), img = range
						.getClosedNode();
				if (img && img.getAttribute('anchorname')) {
					...
					if (name) {
						...
						range.insertNode(anchor).setStartAfter(anchor)
	...
	UE.plugins['pagebreak'] = function() {
					...
					root
							.traversal(function(node) {
								if (node.type == 'text'
									...
									var hr = UE.uNode
											.createElement('<hr class="pagebreak" noshade="noshade" size="5" style="-webkit-user-select: none;">');
		...
		me.commands['pagebreak'] = {
			execCommand : function() {
				var range = me.selection.getRange(), hr = me.document
						.createElement('hr');
				...
				var node = domUtils.findParentByTagName(range.startContainer,
						notBreakTags, true), parents = [], pN;
				if (node) {
					switch (node.tagName) {
					case 'TD':
						pN = node.parentNode;
						if (!pN.previousSibling) {
							var table = domUtils.findParentByTagName(pN,
									'table');
	...
	UE.plugins["wordimage"] = function() {
		...
		me
				.addInputRule(function(root) {
					utils
							.each(
									root.getNodesByTagName('img'),
									function(img) {
										var attrs = img.attrs, flag = parseInt(attrs.width) < 128
												|| parseInt(attrs.height) < 43, opt = me.options, src = opt.UEDITOR_HOME_URL
					...
					function() {
						...
						if (node && node.tagName == 'IMG') {
							...
							if ((pre && pre.nodeType == 1
									&& !domUtils.isEmptyBlock(pre) || !pre)
									&& (!next || next
											&& !domUtils.isEmptyBlock(next))) {
								if (pre && pre.tagName == 'P'
										&& !domUtils.isEmptyBlock(pre)) {
									pre.appendChild(node);
									domUtils.moveChild(next, pre);
									domUtils.remove(next);
								} else if (next && next.tagName == 'P'
										&& !domUtils.isEmptyBlock(next)) {
				...
				.addListener(
						...
						function(type, evt) {
							...
							if (keyCode == 13) {
								...
								if (node = domUtils.findParentByTagName(
										rng.startContainer, 'p', true)) {
									if (domUtils.getComputedStyle(node,
											'text-align') == 'center') {
										domUtils
												.removeStyle(node, 'text-align')
									}
								}
							}
						})
...
//	    me.addListener('afterexeccommand', saveScene);
	...
	UE.plugins['paste'] = function() {
		function getClipboardData(callback) {
			...
			var range = this.selection.getRange(), bk = range.createBookmark(), pastebin = doc
					.createElement('div');
			pastebin.id = 'baidu_pastebin';
			browser.webkit
					&& pastebin.appendChild(doc
							.createTextNode(domUtils.fillChar
									+ domUtils.fillChar));
			...
			setTimeout(
					function() {
						if (browser.webkit) {
							for (var i = 0, pastebins = doc
									.querySelectorAll('#baidu_pastebin'), pi; pi = pastebins[i++];) {
		...
		function filter(div) {
			...
			if (div.firstChild) {
				...
				for (var i = 0, ni; ni = nodes[i++];) {
					if (ni.id == '_baidu_cut_start'
							|| ni.id == '_baidu_cut_end') {
						domUtils.remove(ni);
					}
				}
				if (browser.webkit) {
				...
				html = div.innerHTML;
				//safari 处理粘贴的时候会多一个本地的文件引用 blob  PM#107121
	            if(html && html.indexOf("blob:http")>0){
	                html="";
	            }
				...
				if (me.queryCommandState('pasteplain') === 1) {
					me.execCommand('insertHtml', UE.filterNode(root,
							me.options.filterTxtRules).toHtml(), true);
				} else {
					...
					htmlContent = htmlContent.replace(hostName, "");
		...
		me
				.addListener(
						...
						function(cmd, plainType) {
							if (address && txtContent && htmlContent
									&& txtContent != htmlContent) {
								var range = me.selection.getRange();
								range.moveToAddress(address, true);
								if (!range.collapsed) {
									while (!domUtils
											.isBody(range.startContainer)) {
										var start = range.startContainer;
										if (start.nodeType == 1) {
											start = start.childNodes[range.startOffset];
											if (!start) {
												range
														.setStartBefore(range.startContainer);
												continue;
											}
											var pre = start.previousSibling;
											if (pre
									...
									while (!domUtils.isBody(range.endContainer)) {
										...
										if (end.nodeType == 1) {
											...
											if (!end) {
												range
														.setEndAfter(range.endContainer);
												continue;
											}
								...
								me.__hasEnterExecCommand = false;
								var rng = me.selection.getRange();
								while (!domUtils.isBody(rng.startContainer)
										&& !rng.startOffset
										&& rng.startContainer[rng.startContainer.nodeType == 3 ? 'nodeValue'
												: 'childNodes'].length) {
		...
		me.addListener('ready', function() {
			...
			domUtils.on(me.body, browser.ie || browser.opera ? 'keydown'
					: 'paste', function(e) {
	...
	// chrome 下截图复制到编辑器
	UE.plugins['pasteimage'] = function () {
		var me = this;
		var domUtils = UE.dom.domUtils;
		var browser = UE.browser;
		function uploadImageAndInsertFile(file, editor) {
			var me  = editor;
			var xhr = new XMLHttpRequest(),
				fd = new FormData(),
				uploadUrl = me.getOpt("imageUrl"),
				fileSize = file.size,
				pasteImageMaxSize = me.getOpt("pasteImageMaxSize");

			if(fileSize > pasteImageMaxSize){
				alert("粘贴图片过大，请选择另存文件上传");
				return;
			}

			var mask = domUtils.createElement(top.document, 'div', {
				'style':"position:absolute;top:0;left:0;right:0;bottom:0;z-index:9999;"
			}),tipsWidth=300 ,tipsHeight=60;
			var topV =(window.screen.height - tipsHeight )/2 + window.scrollY;
			var leftV =(window.screen.width - tipsWidth )/2 + window.scrollX;

			var tips = domUtils.createElement(top.document, 'div', {
				'style':"position:absolute;width:" + tipsWidth + "px;height:" + tipsHeight + "px;line-height:"
					+ tipsHeight + "px;text-align:center;border-radius:4px;left:" + leftV + "px;top:" + topV
					+ "px;z-index:10000;background:#000;opacity:0.7;color:#fff;",
				'innerHTML': '正在复制截图...'
			});
			...
			xhr.onload = function () {
				me.isUploadingPasteImage = false;

				top.document.documentElement.removeChild(mask);
				top.document.documentElement.removeChild(tips);
				...
				var img = new Image();
				me.selection.getRange().insertNode(img);
				img.src = data.url;
				me.undoManger.save();
			};

			me.isUploadingPasteImage = true;
			top.document.documentElement.appendChild(mask);
			top.document.documentElement.appendChild(tips);
		...
		function getPasteImage(e){
			...
			var length = e.clipboardData.items.length;
		...
		function dataURLtoBlob(imgData) {
		...
		me.addListener('ready', function () {
			domUtils.on(me.body, browser.ie || browser.opera ? 'keydown' : 'paste', function (e) {
				...
				if(me.options && !me.options.disablePasteImage){
					if (me.isUploadingPasteImage) {
						e.preventDefault();
						return false;
					}
						...
						setTimeout(function(){
					        ...
					        for(var i = 0 ; i < imgs.length; i++ ){
					        	...
					        	if(src && src.indexOf('data:image') > -1){
						        	...
						        	uploadImageAndInsertFile(file,me);
					        	}
					        }
						},100);
						return false;
		    ...
		    if(typeof(me.body.draggable) != 'undefined' && !me.options.disableDraggable){
				me.body.ondrop = function (event) {
					try{
						var files = event.dataTransfer.files;
						for(var i = 0 ; i < files.length;i++){
							var fileItem = files.item(i);
							if(fileItem && fileItem.type.indexOf('image') > -1){
								uploadImageAndInsertFile(fileItem,me);
							}
						}
					}catch(err){}
					return false;
	...
	UE.plugins['list'] = function() {
		...
		for ( var s in customStyle) {
			if (!me.options.insertorderedlist.hasOwnProperty(s)
		...
		me
				.ready(function() {
					...
					for ( var p in customStyle) {
						if (p == 'dash' || p == 'dot') {
							customCss.push('li.list-' + customStyle[p]
									+ '{background-image:url(' + liiconpath
									+ customStyle[p] + '.gif)}');
							customCss
									.push('ul.custom_'
											+ p
							...
							for (var i = 0; i < 99; i++) {
								customCss.push('li.list-' + customStyle[p] + i
										+ '{background-image:url(' + liiconpath
										+ 'list-' + customStyle[p] + i
										+ '.gif)}')
							}
							customCss
									.push('ol.custom_'
					...
					customCss.push('.list-paddingleft-2{padding-left:'
							+ me.options.listDefaultPaddingLeft + 'px}');
					customCss.push('.list-paddingleft-3{padding-left:'
							+ me.options.listDefaultPaddingLeft * 2 + 'px}');
					utils.cssRule('list', 'ol,ul{margin:0;pading:0;'
							+ (browser.ie ? '' : 'width:95%')
		...
		me.ready(function() {
			domUtils.on(me.body, 'cut', function() {
				setTimeout(function() {
					...
					if (!rng.collapsed) {
						if (li = domUtils.findParentByTagName(
								rng.startContainer, 'li', true)) {
		...
		me.addListener('beforepaste', function(type, html) {
			...
			if (li = domUtils.findParentByTagName(rng.startContainer, 'li',
					true)) {
				var list = li.parentNode, tagName = list.tagName == 'OL' ? 'ul'
						: 'ol';
				utils.each(root.getNodesByTagName(tagName), function(n) {
					...
					if (n.parentNode !== root)
						index = index + 1 == listStyle[list.tagName].length ? 0
								: index + 1;
							...
							.each(
									...
									function(node) {
										...
										if (node.parentNode.tagName != 'li'
												&& (type = checkListType(node
														.innerText(), node))) {
											var list = UE.uNode
													.createElement(me.options.insertorderedlist
															.hasOwnProperty(type) ? 'ol'
															: 'ul');
											if (customStyle[type]) {
												list.setAttr('class', 'custom_'
														+ type)
											} else {
												list
														.setStyle(
		...
		function adjustListStyle(doc, ignore) {
			utils
					.each(
							domUtils.getElementsByTagName(doc, 'ol ul'),
							function(node) {
								if (!domUtils.inDoc(node, doc))
									return;
								var parent = node.parentNode;
								if (parent.tagName == node.tagName) {
									var nodeStyleType = getStyle(node)
											|| (node.tagName == 'OL' ? 'decimal'
													: 'disc'), parentStyleType = getStyle(parent)
											|| (parent.tagName == 'OL' ? 'decimal'
													: 'disc');
									if (nodeStyleType == parentStyleType) {
										...
										setListStyle(
												node,
												listStyle[node.tagName][styleIndex])
									}
								}
								var index = 0, type = 2;
								if (domUtils.hasClass(node, /custom_/)) {
									if (!(/[ou]l/i.test(parent.tagName) && domUtils
											.hasClass(parent, /custom_/))) {
										type = 1;
									}
								} else {
									if (/[ou]l/i.test(parent.tagName)
											&& domUtils.hasClass(parent,
													/custom_/)) {
										type = 3;
									}
								}
								var style = domUtils.getStyle(node,
										'list-style-type');
								style
										&& (node.style.cssText = 'list-style-type:'
												+ style);
								node.className = utils.trim(node.className
								...
								!ignore
										&& adjustList(node, node.tagName
												.toLowerCase(), getStyle(node)
												|| domUtils.getStyle(node,
														'list-style-type'),
												true);
							})
		}
		function adjustList(list, tag, style, ignoreEmpty) {
			...
			if (nextList
					...
					&& (getStyle(nextList)
							|| domUtils.getStyle(nextList, 'list-style-type') || (tag == 'ol' ? 'decimal'
							: 'disc')) == style) {
			...
			if (preList
					...
					&& (getStyle(preList)
							|| domUtils.getStyle(preList, 'list-style-type') || (tag == 'ol' ? 'decimal'
							: 'disc')) == style) {
			...
			!ignoreEmpty && domUtils.isEmptyBlock(list)
		...
		me.addListener('keydown', function(type, evt) {
			...
			if (keyCode == 13 && !evt.shiftKey) {
				var rng = me.selection.getRange(), parent = domUtils
						.findParent(rng.startContainer, function(node) {
							return domUtils.isBlockElm(node)
						}, true), li = domUtils.findParentByTagName(
						rng.startContainer, 'li', true);
				if (parent && parent.tagName != 'PRE' && !li) {
					var html = parent.innerHTML.replace(new RegExp(
							domUtils.fillChar, 'g'), '');
				...
				var range = me.selection.getRange(), start = findList(
						range.startContainer, function(node) {
							return node.tagName == 'TABLE';
						}), end = range.collapsed ? start : findList(
						range.endContainer, function(node) {
							return node.tagName == 'TABLE';
						});
				if (start && end && start === end) {
					if (!range.collapsed) {
						start = domUtils.findParentByTagName(
								range.startContainer, 'li', true);
						end = domUtils.findParentByTagName(range.endContainer,
								'li', true);
						if (start && end && start === end) {
							range.deleteContents();
							...
							if (li && domUtils.isEmptyBlock(li)) {
								...
								if (pre && next) {
									...
									if (!pre && !next || !pre) {
										parentList.parentNode.insertBefore(p,
												parentList);
									} else {
										li.parentNode.parentNode.insertBefore(
												p, parentList.nextSibling);
									}
									domUtils.remove(li);
							...
							var tmpRange = range.cloneRange(), bk = tmpRange
									.collapse(false).createBookmark();
							range.deleteContents();
							tmpRange.moveToBookmark(bk);
							var li = domUtils.findParentByTagName(
									tmpRange.startContainer, 'li', true);
					...
					li = domUtils.findParentByTagName(range.startContainer,
							'li', true);
					if (li) {
						if (domUtils.isEmptyBlock(li)) {
							...
							if (li !== parentList.lastChild) {
								...
								parentList.parentNode.insertBefore(li,
							...
							range.setStart(first, 0).collapse(true)
			...
			if (keyCode == 8) {
				...
				if (range.collapsed && domUtils.isStartInblock(range)) {
					...
					if (li && domUtils.isStartInblock(tmpRange)) {
						start = domUtils.findParentByTagName(
								range.startContainer, 'p', true);
						if (start && start !== li.firstChild) {
							var parentList = domUtils.findParentByTagName(
									start, [ 'ol', 'ul' ]);
						...
						if (li && (pre = li.previousSibling)) {
							...
							if (domUtils.isBlockElm(first)) {
								if (domUtils.isEmptyNode(first)) {
									...
									range.setStart(first, 0).setCursor(false,
											true);
									while (li.firstChild) {
									...
									range.setStartBefore(span).collapse(true)
											.select(true);
									domUtils.remove(span);
								}
							} else {
						...
						if (li && !li.previousSibling) {
							...
							if (domUtils.isTagNode(parentList.parentNode,
									'ol ul')) {
								parentList.parentNode.insertBefore(li,
										parentList);
								if (domUtils.isEmptyNode(parentList)) {
		...
		me.addListener('keyup', function(type, evt) {
			...
			if (keyCode == 8) {
				...
				if (list = domUtils.findParentByTagName(rng.startContainer, [
						'ol', 'ul' ], true)) {
					adjustList(list, list.tagName.toLowerCase(), getStyle(list)
							|| domUtils.getComputedStyle(list,
						...
						function() {
							...
							if (li) {
								...
								if (range.collapsed) {
									if (checkLevel(li))
										return true;
									var parentLi = li.parentNode, list = me.document
											.createElement(parentLi.tagName), index = utils
											.indexOf(
									...
									if (domUtils.isStartInblock(range)) {
										...
										list.appendChild(li);
										adjustList(list, list.tagName
												.toLowerCase(), currentStyle);
										me.fireEvent('contentchange');
										range.moveToBookmark(bk).select(true);
										return true;
									}
									...
									me.fireEvent('saveScene');
									bk = range.createBookmark();
									for (var i = 0, closeList, parents = domUtils
											.findParents(li), ci; ci = parents[i++];) {
									...
									if (bk.end) {
										while (current
											...
											var parentLi = current.parentNode, list = me.document
													.createElement(parentLi.tagName), index = utils
													.indexOf(
															listStyle[list.tagName],
															getStyle(parentLi)
																	|| domUtils
																			.getComputedStyle(
																					parentLi,
																					'list-style-type'));
											var currentIndex = index + 1 == listStyle[list.tagName].length ? 0
													: index + 1;
											var currentStyle = listStyle[list.tagName][currentIndex];
											setListStyle(list, currentStyle);
											parentLi
													.insertBefore(list, current);
											while (current
													&& !(domUtils.getPosition(
															current, bk.end) & domUtils.POSITION_FOLLOWING)) {
		...
		me.commands['insertorderedlist'] = me.commands['insertunorderedlist'] = {
			execCommand : function(command, style) {
				if (!style) {
					style = command.toLowerCase() == 'insertorderedlist' ? 'decimal'
							: 'disc';
				}
				var me = this, range = this.selection.getRange(), filterFn = function(
						node) {
					return node.nodeType == 1 ? node.tagName.toLowerCase() != 'br'
							: !domUtils.isWhitespace(node);
				}, tag = command.toLowerCase() == 'insertorderedlist' ? 'ol'
						: 'ul', frag = me.document.createDocumentFragment();
				range.adjustmentBoundary().shrinkBoundary();
				var bko = range.createBookmark(true), start = getLi(me.document
						.getElementById(bko.start)), modifyStart = 0, end = getLi(me.document
						.getElementById(bko.end)), modifyEnd = 0, startParent, endParent, list, tmp;
				if (start || end) {
					...
					if (startParent === endParent) {
						...
						var nodeStyle = getStyle(startParent)
								|| domUtils.getComputedStyle(startParent,
										'list-style-type')
								|| (command.toLowerCase() == 'insertorderedlist' ? 'decimal'
										: 'disc');
						if (startParent.tagName.toLowerCase() == tag
								&& nodeStyle == style) {
							for (var i = 0, ci, tmpFrag = me.document
									.createDocumentFragment(); ci = frag.childNodes[i++];) {
								if (domUtils.isTagNode(ci, 'ol ul')) {
					...
					if (start) {
						while (start) {
							...
							if (domUtils.isTagNode(start, 'ol ul')) {
								...
								var tmpfrag = me.document
										.createDocumentFragment(), hasBlock = 0;
						...
						startParent.parentNode.insertBefore(frag,
				...
				var bk = range.createBookmark(), current = domUtils
						.getNextDomNode(bk.start, false, filterFn), tmpRange = range
						.cloneRange(), tmpNode, block = domUtils.isBlockElm;
				while (current
						&& current !== bk.end
					...
					if (current.nodeType == 3 || dtd.li[current.tagName]) {
						if (current.nodeType == 1 && dtd.$list[current.tagName]) {
							...
							tmpNode = domUtils.getNextDomNode(current, false,
									filterFn);
						...
						while (current
								&& current !== bk.end
								&& (!block(current) || domUtils
										.isBookmarkNode(current))) {
							tmpNode = current;
							current = domUtils.getNextDomNode(current, false,
									null, function(node) {
										return !notExchange[node.tagName];
									});
						}
						if (current && block(current)) {
							tmp = domUtils.getNextDomNode(tmpNode, false,
									filterFn);
							if (tmp && domUtils.isBookmarkNode(tmp)) {
								current = domUtils.getNextDomNode(tmp, false,
										filterFn);
								tmpNode = tmp;
							}
						}
						tmpRange.setEndAfter(tmpNode);
						current = domUtils.getNextDomNode(tmpNode, false,
								filterFn);
			...
			queryCommandState : function(command) {
				var tag = command.toLowerCase() == 'insertorderedlist' ? 'ol'
						: 'ul';
				var path = this.selection.getStartElementPath();
				for (var i = 0, ci; ci = path[i++];) {
			...
			queryCommandValue : function(command) {
				...
				return node ? getStyle(node)
						|| domUtils.getComputedStyle(node, 'list-style-type')
						: null;
			}
		};
	};
	(function() {
		...
		UE.plugins['source'] = function() {
			...
			var sourceEditor;
			opt.sourceEditor = browser.ie ? 'textarea'
					: (opt.sourceEditor || 'codemirror');
			me.setOpt({
				sourceEditorFirst : false
			});
			function createSourceEditor(holder) {
				return sourceEditors[opt.sourceEditor == 'codemirror'
						&& window.CodeMirror ? 'codemirror' : 'textarea'](me,
						holder);
			}
			var bakCssText;
			...
			me.commands['source'] = {
				execCommand : function() {
					...
					if (sourceMode) {
						bakAddress = me.selection.getRange().createAddress(
								false, true);
						...
						me.getContent = function() {
							return sourceEditor.getContent() || '<p>'
									+ (browser.ie ? '' : '<br/>') + '</p>';
						};
					} else {
						me.iframe.style.cssText = bakCssText;
						var cont = sourceEditor.getContent() || '<p>'
								+ (browser.ie ? '' : '<br/>') + '</p>';
						cont = cont
								.replace(
										new RegExp(
							...
							if (keyCode == 13) {
								...
								if (!range.collapsed) {
									var start = range.startContainer, end = range.endContainer, startTd = domUtils
											.findParentByTagName(start, 'td',
													true), endTd = domUtils
											.findParentByTagName(end, 'td',
													true);
									if (startTd && endTd && startTd !== endTd
											|| !startTd && endTd || startTd
											&& !endTd) {
								...
								if (tag == 'p') {
									...
									if (!range.collapsed) {
										...
										if (start.nodeType == 1
											...
											br = range.document
													.createElement('br');
											range.insertNode(br).setStartAfter(
													br).setCursor();
										}
									} else {
	...
	UE.plugins['keystrokes'] = function() {
		...
		me
				.addListener(
						'keydown',
						function(type, evt) {
							var keyCode = evt.keyCode || evt.which, rng = me.selection
									.getRange();
							if (!rng.collapsed
								...
								if (rng.startOffset == 0) {
									...
									if (domUtils.isBoundaryNode(tmpNode,
											'firstChild')) {
										tmpNode = rng.endContainer;
										if (rng.endOffset == (tmpNode.nodeType == 3 ? tmpNode.nodeValue.length
												: tmpNode.childNodes.length)
												&& domUtils.isBoundaryNode(
														tmpNode, 'lastChild')) {
											me.fireEvent('saveScene');
											me.body.innerHTML = '<p>'
													+ (browser.ie ? ''
															: '<br/>') + '</p>';
											rng.setStart(me.body.firstChild, 0)
													.setCursor(false, true);
											me._selectionChange();
											return;
										}
							...
							if (keyCode == 8) {
								...
								if (rng.collapsed && rng.inFillChar()) {
									...
									if (domUtils.isFillChar(start)) {
										rng.setStartBefore(start)
												.shrinkBoundary(true).collapse(
														true);
										domUtils.remove(start)
									} else {
										start.nodeValue = start.nodeValue
												.replace(new RegExp('^'
														+ domUtils.fillChar),
														'');
										rng.startOffset--;
								...
								if (!browser.ie) {
									start = domUtils.findParentByTagName(
											rng.startContainer, 'table', true);
									end = domUtils.findParentByTagName(
											rng.endContainer, 'table', true);
									if (start && !end || !start && end
											|| start !== end) {
										evt.preventDefault();
										return;
							...
							if (keyCode == 9) {
								...
								var range = me.selection.getRange();
								me.fireEvent('saveScene');
								for (var i = 0, txt = '', tabSize = me.options.tabSize || 4, tabNode = me.options.tabNode
										|| '&nbsp;'; i < tabSize; i++) {
									txt += tabNode;
								}
								var span = me.document.createElement('span');
								...
								if (range.collapsed) {
									...
									start = domUtils.findParent(
											range.startContainer, filterFn);
									end = domUtils.findParent(
											range.endContainer, filterFn);
									if (start && end && start === end) {
										range.deleteContents();
										range
												.insertNode(
										...
										var bookmark = range.createBookmark(), filterFn = function(
												node) {
											return domUtils.isBlockElm(node)
													&& !excludeTagNameForTabKey[node.tagName
															.toLowerCase()]
										};
										range.enlarge(true);
										var bookmark2 = range.createBookmark(), current = domUtils
												.getNextDomNode(
														bookmark2.start, false,
														filterFn);
										while (current
												&& !(domUtils.getPosition(
														current, bookmark2.end) & domUtils.POSITION_FOLLOWING)) {
											current
													.insertBefore(
															span
							...
							if (browser.gecko && keyCode == 46) {
								...
								if (range.collapsed) {
									...
									if (domUtils.isEmptyBlock(start)) {
										...
										while (domUtils.getChildCount(parent) == 1
				...
				.addListener(
						...
						function(type, evt) {
							...
							if (keyCode == 8) {
								...
								rng = me.selection.getRange();
								if (rng.collapsed) {
									var tmpNode, autoClearTagName = [ 'h1',
											'h2', 'h3', 'h4', 'h5', 'h6' ];
									if (tmpNode = domUtils.findParentByTagName(
											rng.startContainer,
											autoClearTagName, true)) {
										if (domUtils.isEmptyBlock(tmpNode)) {
											var pre = tmpNode.previousSibling;
											if (pre && pre.nodeName != 'TABLE') {
												domUtils.remove(tmpNode);
												rng.setStartAtLast(pre)
														.setCursor(false, true);
												return;
											} else {
												var next = tmpNode.nextSibling;
									...
									if (domUtils.isBody(rng.startContainer)) {
										...
										rng.insertNode(tmpNode).setStart(
												tmpNode, 0).setCursor(false,
												true);
									}
								}
								if (!collapsed
										&& (rng.startContainer.nodeType == 3 || rng.startContainer.nodeType == 1
												&& domUtils
														.isEmptyBlock(rng.startContainer))) {
									if (browser.ie) {
										var span = rng.document
												.createElement('span');
										rng.insertNode(span).setStartBefore(
												span).collapse(true);
										rng.select();
										domUtils.remove(span)
									} else {
				...
				.addListener(
						...
						function(type, evt) {
							...
							if (keyCode == 32 || keyCode == 13) {
								var sel = me.selection.getNative(), range = sel
										.getRangeAt(0).cloneRange(), offset, charCode;
								var start = range.startContainer;
								while (start.nodeType == 1
										&& range.startOffset > 0) {
									...
									range
											.setStart(
													start,
													start.nodeType == 1 ? start.childNodes.length
								...
								if (range
									...
									if (domUtils.findParentByTagName(
											range.startContainer, 'a', true)) {
										return;
									}
									var a = me.document.createElement('a'), text = me.document
											.createTextNode(' '), href;
									me.undoManger && me.undoManger.save();
									...
									href = a.getAttribute("href").replace(
											...
											'');
									href = /^(?:https?:\/\/)/ig.test(href) ? href
											: "http://" + href;
									a.setAttribute('_src', utils.html(href));
									a.href = utils.html(href);
									range.insertNode(a);
									a.parentNode.insertBefore(text,
											a.nextSibling);
	...
	UE.plugins['autoheight'] = function() {
		...
		function adjustHeight() {
			...
			timer = setTimeout(
					function() {
						if (!me.queryCommandState || me.queryCommandState
								&& me.queryCommandState('source') != 1) {
							if (!span) {
								span = me.document.createElement('span');
							...
							currentHeight = Math.max(domUtils.getXY(tmpNode).y
									+ tmpNode.offsetHeight, Math.max(
		...
		me.addListener('destroy', function() {
			me.removeListener('contentchange afterinserthtml keyup mouseup',
					adjustHeight)
		});
		me.enableAutoHeight = function() {
			...
			me.addListener('contentchange afterinserthtml keyup mouseup',
					adjustHeight);
			setTimeout(function() {
		...
		me.addListener('ready', function() {
			me.enableAutoHeight();
			var timer;
			domUtils.on(browser.ie ? me.body : me.document,
					browser.webkit ? 'dragover' : 'drop', function() {
						clearTimeout(timer);
						timer = setTimeout(function() {
	...
	UE.plugins['autofloat'] = function() {
		...
		var uiUtils = UE.ui.uiUtils, LteIE6 = browser.ie
				&& browser.version <= 6, quirks = browser.quirks;
		function checkHasUI() {
			if (!UE.ui) {
		...
		function setFloating() {
			var toobarBoxPos = domUtils.getXY(toolbarBox), origalFloat = domUtils
					.getComputedStyle(toolbarBox, 'position'), origalLeft = domUtils
					.getComputedStyle(toolbarBox, 'left');
			...
			if (LteIE6 || (quirks && browser.ie)) {
				...
				if (browser.ie7Compat && flag) {
					flag = false;
					toolbarBox.style.left = domUtils.getXY(toolbarBox).x
	...
	UE.plugins['video'] = function() {
		...
		function switchImgAndEmbed(root, img2embed) {
			utils.each(root.getNodesByTagName(img2embed ? 'img' : 'embed'),
					function(node) {
						if (node.getAttr('class') == 'edui-faked-video') {
							var html = creatInsertStr(img2embed ? node
									.getAttr('_url') : node.getAttr('src'),
									node.getAttr('width'), node
											.getAttr('height'), null, node
											.getStyle('float')
											|| '', img2embed);
							node.parentNode.replaceChild(UE.uNode
									.createElement(html), node)
						}
					})
		}
		me.addOutputRule(function(root) {
		...
		me.commands["insertvideo"] = {
			execCommand : function(cmd, videoObjs) {
				videoObjs = utils.isArray(videoObjs) ? videoObjs
						: [ videoObjs ];
				var html = [], id = 'tmpVedio';
				for (var i = 0, vi, len = videoObjs.length; i < len; i++) {
					vi = videoObjs[i];
					html.push(creatInsertStr(vi.url, vi.width || 420,
							vi.height || 280, id + i, null, false));
				}
			...
			queryCommandState : function() {
				var img = me.selection.getRange().getClosedNode(), flag = img
						&& (img.className == "edui-faked-video");
				return flag ? 1 : 0;
			}
		};
	...
	(function() {
		...
		UETable.isEmptyBlock = function(node) {
			var reg = new RegExp(domUtils.fillChar, 'g');
			if (node[browser.ie ? 'innerText' : 'textContent'].replace(/^\s*$/,
		...
		UETable.getTableItemsByRange = function(editor) {
			...
			var cell = start
					&& domUtils
							.findParentByTagName(start, [ "td", "th" ], true), tr = cell
					&& cell.parentNode, caption = start
					&& domUtils.findParentByTagName(start, 'caption', true), table = caption ? caption.parentNode
		...
		UETable.getDefaultValue = function(editor, table) {
			...
			if (!table) {
				...
				tmpValue = domUtils
						.getComputedStyle(table, 'border-left-width');
		...
		UETable.getUETable = function(tdOrTable) {
			...
			tdOrTable = (tag == "td" || tag == "th" || tag == 'caption') ? domUtils
					.findParentByTagName(tdOrTable, "table", true)
					: tdOrTable;
		...
		UETable.prototype = {
			...
			getHSideCell : function(cell, right) {
				try {
					...
					if ((!right && (!len ? !cellInfo.colIndex
							: !range.beginColIndex))
							|| (right && (!len ? (cellInfo.colIndex == (this.colsNum - 1))
									: (range.endColIndex == this.colsNum - 1))))
						return null;
					previewRowIndex = !len ? cellInfo.rowIndex
							: range.beginRowIndex;
					previewColIndex = !right ? (!len ? (cellInfo.colIndex < 1 ? 0
							: (cellInfo.colIndex - 1))
							: range.beginColIndex - 1)
							: (!len ? cellInfo.colIndex + 1
									: range.endColIndex + 1);
					return this
							.getCell(
			...
			getTabNextCell : function(cell, preRowIndex) {
				var cellInfo = this.getCellInfo(cell), rowIndex = preRowIndex
						|| cellInfo.rowIndex, colIndex = cellInfo.colIndex + 1
						+ (cellInfo.colSpan - 1), nextCell;
				try {
			...
			getVSideCell : function(cell, bottom, ignoreRange) {
				try {
					...
					if ((!bottom && (cellInfo.rowIndex == 0))
							|| (bottom && (!len ? (cellInfo.rowIndex
									+ cellInfo.rowSpan > this.rowsNum - 1)
									: (range.endRowIndex == this.rowsNum - 1))))
						return null;
					nextRowIndex = !bottom ? (!len ? cellInfo.rowIndex - 1
							: range.beginRowIndex - 1)
							: (!len ? (cellInfo.rowIndex + cellInfo.rowSpan)
									: range.endRowIndex + 1);
					nextColIndex = !len ? cellInfo.colIndex
							: range.beginColIndex;
					return this
							.getCell(
			...
			getSameEndPosCells : function(cell, xOrY) {
				try {
					var flag = (xOrY.toLowerCase() === "x"), end = domUtils
							.getXY(cell)[flag ? 'x' : 'y']
							+ cell["offset" + (flag ? 'Width' : 'Height')], rows = this.table.rows, cells = null, returns = [];
					for (var i = 0; i < this.rowsNum; i++) {
						cells = rows[i].cells;
						for (var j = 0, tmpCell; tmpCell = cells[j++];) {
							var tmpEnd = domUtils.getXY(tmpCell)[flag ? 'x'
									: 'y']
									+ tmpCell["offset"
											+ (flag ? 'Width' : 'Height')];
							if (tmpEnd > end && flag)
			...
			update : function(table) {
				...
				this.cellsRange = {};
				this.indexTable = [];
				var rows = this.table.rows, rowsNum = this.getMaxRows(), dNum = rowsNum
						- rows.length, colsNum = this.getMaxCols();
				while (dNum--) {
				...
				for (j = 0; j < rowsNum; j++) {
					for (k = 0; k < colsNum; k++) {
						if (this.indexTable[j][k] === undefined) {
							...
							cell = cell ? cell.cloneNode(true)
									: this.table.ownerDocument
											.createElement("td");
				...
				if (selectTds.length) {
					var start = selectTds[0], end = selectTds[selectTds.length - 1], startInfo = this
							.getCellInfo(start), endInfo = this
							.getCellInfo(end);
			...
			getCell : function(rowIndex, cellIndex) {
				return rowIndex < this.rowsNum
						&& this.table.rows[rowIndex].cells[cellIndex] || null;
			},
			deleteCell : function(cell, rowIndex) {
				rowIndex = typeof rowIndex == 'number' ? rowIndex
						: cell.parentNode.rowIndex;
				var row = this.table.rows[rowIndex];
				row.deleteCell(cell.cellIndex);
			},
			getCellsRange : function(cellA, cellB) {
				function checkRange(beginRowIndex, beginColIndex, endRowIndex,
						endColIndex) {
					...
					if (beginRowIndex > 0) {
						for (colIndex = beginColIndex; colIndex < endColIndex; colIndex++) {
							...
							if (rowIndex < beginRowIndex) {
								tmpBeginRowIndex = Math.min(rowIndex,
										tmpBeginRowIndex);
							}
						}
					}
					if (endColIndex < me.colsNum) {
						for (rowIndex = beginRowIndex; rowIndex < endRowIndex; rowIndex++) {
							...
							if (colIndex > endColIndex) {
								tmpEndColIndex = Math.max(colIndex,
										tmpEndColIndex);
							}
						}
					}
					if (endRowIndex < me.rowsNum) {
						for (colIndex = beginColIndex; colIndex < endColIndex; colIndex++) {
							...
							if (rowIndex > endRowIndex) {
								tmpEndRowIndex = Math.max(rowIndex,
										tmpEndRowIndex);
							}
						}
					}
					if (beginColIndex > 0) {
						for (rowIndex = beginRowIndex; rowIndex < endRowIndex; rowIndex++) {
							...
							if (colIndex < beginColIndex) {
								tmpBeginColIndex = Math.min(cellInfo.colIndex,
										tmpBeginColIndex);
							}
						}
					}
					if (tmpBeginRowIndex != beginRowIndex
							|| tmpBeginColIndex != beginColIndex
							|| tmpEndRowIndex != endRowIndex
							|| tmpEndColIndex != endColIndex) {
						return checkRange(tmpBeginRowIndex, tmpBeginColIndex,
				...
				try {
					...
					if (cellA === cellB) {
						return {
							...
							endRowIndex : cellAInfo.rowIndex
									+ cellAInfo.rowSpan - 1,
							endColIndex : cellAInfo.colIndex
									+ cellAInfo.colSpan - 1
						};
					}
					var cellBInfo = me.getCellInfo(cellB);
					var beginRowIndex = Math.min(cellAInfo.rowIndex,
							cellBInfo.rowIndex), beginColIndex = Math.min(
							cellAInfo.colIndex, cellBInfo.colIndex), endRowIndex = Math
							.max(cellAInfo.rowIndex + cellAInfo.rowSpan - 1,
									cellBInfo.rowIndex + cellBInfo.rowSpan - 1), endColIndex = Math
							.max(cellAInfo.colIndex + cellAInfo.colSpan - 1,
			...
			getCells : function(range) {
				...
				for (var i = beginRowIndex; i <= endRowIndex; i++) {
					for (var j = beginColIndex; j <= endColIndex; j++) {
						...
						returnTds.push(this.getCell(rowIndex,
								cellInfo.cellIndex));
					}
				}
				return returnTds;
			...
			isFullCol : function() {
				var range = this.cellsRange, table = this.table, ths = table
						.getElementsByTagName("th"), rows = range.endRowIndex
						- range.beginRowIndex + 1;
				return !ths.length ? rows == this.rowsNum
			...
			getPreviewCell : function(cell, top) {
				try {
					var cellInfo = this.getCellInfo(cell), previewRowIndex, previewColIndex;
					var len = this.selectedTds.length, range = this.cellsRange;
					if ((!top && (!len ? !cellInfo.colIndex
							: !range.beginColIndex))
							|| (top && (!len ? (cellInfo.rowIndex > (this.colsNum - 1))
									: (range.endColIndex == this.colsNum - 1))))
						return null;
					previewRowIndex = !top ? (!len ? cellInfo.rowIndex
							: range.beginRowIndex)
							: (!len ? (cellInfo.rowIndex < 1 ? 0
									: (cellInfo.rowIndex - 1))
									: range.beginRowIndex);
					previewColIndex = !top ? (!len ? (cellInfo.colIndex < 1 ? 0
							: (cellInfo.colIndex - 1))
							: range.beginColIndex - 1)
			...
			moveContent : function(cellTo, cellFrom) {
				...
				if (child.nodeType == 3 || !dtd.$block[child.tagName]) {
					cellTo
							.appendChild(cellTo.ownerDocument
									.createElement('br'))
				}
			...
			mergeRight : function(cell) {
				var cellInfo = this.getCellInfo(cell), rightColIndex = cellInfo.colIndex
						+ cellInfo.colSpan, rightCellInfo = this.indexTable[cellInfo.rowIndex][rightColIndex], rightCell = this
						.getCell(rightCellInfo.rowIndex,
			...
			mergeDown : function(cell) {
				var cellInfo = this.getCellInfo(cell), downRowIndex = cellInfo.rowIndex
						+ cellInfo.rowSpan, downCellInfo = this.indexTable[downRowIndex][cellInfo.colIndex], downCell = this
						.getCell(downCellInfo.rowIndex, downCellInfo.cellIndex);
			...
			mergeRange : function() {
				var range = this.cellsRange, leftTopCell = this
						.getCell(
								range.beginRowIndex,
				...
				if (leftTopCell.tagName == "TH"
					...
					var index = this.indexTable, info = this
							.getCellInfo(leftTopCell);
					leftTopCell = this.getCell(1,
							index[1][info.colIndex].cellIndex);
					range = this.getCellsRange(leftTopCell, this.getCell(
				...
				if (leftTopCell.rowSpan == this.rowsNum
						&& leftTopCell.colSpan != 1) {
					leftTopCell.colSpan = 1;
				}
				if (leftTopCell.colSpan == this.colsNum
						&& leftTopCell.rowSpan != 1) {
			...
			insertRow : function(rowIndex, sourceCell) {
				var numCols = this.colsNum, table = this.table, row = table
						.insertRow(rowIndex), cell, width = parseInt(
						(table.offsetWidth - numCols * 20 - numCols - 1)
								/ numCols, 10);
				if (rowIndex == 0 || rowIndex == this.rowsNum) {
					...
					for (colIndex = 0; colIndex < numCols; colIndex++) {
						...
						if (cellInfo.rowIndex < rowIndex) {
							cell = this.getCell(cellInfo.rowIndex,
									cellInfo.cellIndex);
			...
			deleteRow : function(rowIndex) {
				...
				for (var colIndex = 0; colIndex < colsNum;) {
					var cellInfo = infoRow[colIndex], cell = this.getCell(
					...
					if (cell.rowSpan > 1) {
						if (cellInfo.rowIndex == rowIndex) {
							...
							clone.innerHTML = "";
							cell.rowSpan = 1;
							var nextRowIndex = rowIndex + 1, nextRow = this.table.rows[nextRowIndex], insertCellIndex, preMerged = this
									.getPreviewMergedCellsNum(nextRowIndex,
											colIndex)
									- count;
							if (preMerged < colIndex) {
								insertCellIndex = colIndex - preMerged - 1;
								domUtils.insertAfter(
										nextRow.cells[insertCellIndex], clone);
							} else {
								if (nextRow.cells.length)
									nextRow.insertBefore(clone,
											nextRow.cells[0])
							}
							count += 1;
						}
				...
				for (colIndex = 0; colIndex < colsNum; colIndex++) {
					var tmpRowIndex = infoRow[colIndex].rowIndex, tmpCellIndex = infoRow[colIndex].cellIndex, key = tmpRowIndex
							+ "_" + tmpCellIndex;
					if (cacheMap[key])
						continue;
					cacheMap[key] = 1;
			...
			insertCol : function(colIndex, sourceCell, defaultValue) {
				var rowsNum = this.rowsNum, rowIndex = 0, tableRow, cell, backWidth = parseInt(
				...
				if (colIndex == 0 || colIndex == this.colsNum) {
					for (; rowIndex < rowsNum; rowIndex++) {
						...
						preCell = tableRow.cells[colIndex == 0 ? colIndex
								: tableRow.cells.length];
						cell = this.cloneCell(sourceCell, true);
						this.setCellContent(cell);
						cell
								.setAttribute('vAlign', cell
										.getAttribute('vAlign'));
						preCell
								&& cell.setAttribute('width', preCell
										.getAttribute('width'));
						if (!colIndex) {
					...
					for (; rowIndex < rowsNum; rowIndex++) {
						...
						if (cellInfo.colIndex < colIndex) {
							...
							preCell
									&& cell.setAttribute('width', preCell
											.getAttribute('width'));
							preCell ? tableRow.insertBefore(cell, preCell)
			...
			updateWidth : function(width, defaultValue) {
				var table = this.table, tmpWidth = UETable.getWidth(table)
						- defaultValue.tdPadding * 2 - defaultValue.tdBorder
						+ width;
			...
			deleteCol : function(colIndex) {
				var indexTable = this.indexTable, tableRows = this.table.rows, backTableWidth = this.table
						.getAttribute("width"), backTdWidth = 0, rowsNum = this.rowsNum, cacheMap = {};
				for (var rowIndex = 0; rowIndex < rowsNum;) {
					var infoRow = indexTable[rowIndex], cellInfo = infoRow[colIndex], key = cellInfo.rowIndex
							+ '_' + cellInfo.colIndex;
					if (cacheMap[key])
						continue;
					cacheMap[key] = 1;
					var cell = this.getCell(cellInfo.rowIndex,
							cellInfo.cellIndex);
					if (!backTdWidth)
						backTdWidth = cell
								&& parseInt(cell.offsetWidth / cell.colSpan, 10)
										.toFixed(0);
					if (cell.colSpan > 1) {
						cell.colSpan--;
					} else {
			...
			splitToRows : function(cell) {
				...
				for (var i = rowIndex, endRow = rowIndex + cellInfo.rowSpan; i < endRow; i++) {
					if (i == rowIndex)
						continue;
					var tableRow = this.table.rows[i], tmpCell = tableRow
							.insertCell(colIndex
									- this
											.getPreviewMergedCellsNum(i,
													colIndex));
			...
			splitToCols : function(cell) {
				var backWidth = (cell.offsetWidth / cell.colSpan - 22)
						.toFixed(0), cellInfo = this.getCellInfo(cell), rowIndex = cellInfo.rowIndex, colIndex = cellInfo.colIndex, results = [];
				cell.colSpan = 1;
				...
				for (var j = colIndex, endCol = colIndex + cellInfo.colSpan; j < endCol; j++) {
					if (j == colIndex)
						continue;
					var tableRow = this.table.rows[rowIndex], tmpCell = tableRow
							.insertCell(this.indexTable[rowIndex][j].cellIndex + 1);
			...
			selectRow : function(rowIndex) {
				var indexRow = this.indexTable[rowIndex], start = this.getCell(
						indexRow[0].rowIndex, indexRow[0].cellIndex), end = this
						.getCell(indexRow[this.colsNum - 1].rowIndex,
								indexRow[this.colsNum - 1].cellIndex), range = this
						.getCellsRange(start, end);
				this.setSelected(range);
			},
			selectTable : function() {
				var tds = this.table.getElementsByTagName("td"), range = this
						.getCellsRange(tds[0], tds[tds.length - 1]);
				this.setSelected(range);
			},
			sortTable : function(sortByCellIndex, compareFn) {
				...
				if (this.selectedTds.length) {
					...
					lastRowIndex = (range.endRowIndex + 1) === this.rowsNum ? 0
							: range.endRowIndex + 1;
				} else {
					for (var i = 0, len = rows.length; i < len; i++) {
						trArray[i] = rows[i];
				...
				trArray = utils
						.sort(
								...
								function(tr1, tr2) {
									...
									return compareFn ? (typeof compareFn === "number" ? compareFn
											: compareFn.call(this,
				...
				if (!lastRowIndex) {
					...
					tbody.insertBefore(fragment, rows[lastRowIndex
							- range.endRowIndex + range.beginRowIndex - 1])
				}
			},
			setBackground : function(cells, value) {
				if (typeof value === "string") {
					...
					var rowIndex = this.getCellInfo(cells[0]).rowIndex, count = 0, colors = value.colorList, getColor = function(
							list, index, repeat) {
						return list[index] ? list[index] : repeat ? list[index
								% list.length] : "";
					};
					for (var i = 0, cell; cell = cells[i++];) {
						...
						cell.style.backgroundColor = getColor(
								colors,
								((rowIndex + count) == cellInfo.rowIndex) ? count
										: ++count, value.repeat);
					}
				}
			},
	...
	(function() {
		...
		UE.commands['inserttable'] = {
			...
			execCommand: function (cmd, opt) {
				function createTable(opt, tableWidth, tdWidth) {
					...
					return '<table class="editor-table" width="' + tableWidth + '"   ><tbody>' + html.join('') + '</tbody></table>'
				}
		...
		UE.commands['deletetable'] = {
			queryCommandState : function() {
				var rng = this.selection.getRange();
				return domUtils.findParentByTagName(rng.startContainer,
						'table', true) ? 0 : -1;
			},
			execCommand : function(cmd, table) {
				var rng = this.selection.getRange();
				table = table
						|| domUtils.findParentByTagName(rng.startContainer,
								'table', true);
				if (table) {
		...
		UE.commands['insertcaption'] = {
			queryCommandState : function() {
				var table = getTableItemsByRange(this).table;
				if (table) {
					return table.getElementsByTagName('caption').length == 0 ? 1
							: -1;
				}
				return -1;
			},
		...
		UE.commands['deletecaption'] = {
			queryCommandState : function() {
				var rng = this.selection.getRange(), table = domUtils
						.findParentByTagName(rng.startContainer, 'table');
				if (table) {
					return table.getElementsByTagName('caption').length == 0 ? -1
							: 1;
				}
		...
		UE.commands['inserttitle'] = {
			queryCommandState : function() {
				...
				if (table) {
					...
					return firstRow.getElementsByTagName('th').length == 0 ? 0
							: -1
				}
				return -1;
			},
			execCommand : function() {
				...
				this.selection.getRange().setStart(th, 0)
						.setCursor(false, true);
			}
		};
		UE.commands['deletetitle'] = {
			...
			execCommand : function() {
				...
				var td = table.getElementsByTagName('td')[0];
				this.selection.getRange().setStart(td, 0)
						.setCursor(false, true);
			}
		};
		UE.commands["mergeright"] = {
			queryCommandState : function(cmd) {
				...
				if (ut.selectedTds.length)
					return -1;
				var cellInfo = ut.getCellInfo(tableItems.cell), rightColIndex = cellInfo.colIndex
						+ cellInfo.colSpan;
			...
			execCommand : function(cmd) {
				var rng = this.selection.getRange(), bk = rng
						.createBookmark(true);
		...
		UE.commands["mergedown"] = {
			queryCommandState : function(cmd) {
				...
				if (ut.selectedTds.length)
					return -1;
				var cellInfo = ut.getCellInfo(tableItems.cell), downRowIndex = cellInfo.rowIndex
						+ cellInfo.rowSpan;
				if (downRowIndex >= ut.rowsNum)
					return -1;
				var downCellInfo = ut.indexTable[downRowIndex][cellInfo.colIndex];
				return (downCellInfo.colIndex == cellInfo.colIndex && downCellInfo.colSpan == cellInfo.colSpan)
						&& tableItems.cell.tagName !== 'TH' ? 0 : -1;
			},
			execCommand : function() {
				var rng = this.selection.getRange(), bk = rng
						.createBookmark(true);
			...
			execCommand : function(cmd) {
				...
				if (table) {
					if (cmd == 'adaptbywindow') {
						...
						var cells = domUtils.getElementsByTagName(table,
								"td th");
						utils.each(cells, function(cell) {
							cell.removeAttribute("width");
						});
		...
		UE.commands['averagedistributecol'] = {
			...
			execCommand : function(cmd) {
				...
				function getAverageWidth() {
					var tb = ut.table, averageWidth, sumWidth = 0, colsNum = 0, tbAttr = getDefaultValue(
							me, tb);
					if (ut.isFullRow()) {
					...
					averageWidth = Math.ceil(sumWidth / colsNum)
							- tbAttr.tdBorder * 2 - tbAttr.tdPadding * 2;
					return averageWidth;
				}
				function setAverageWidth(averageWidth) {
					utils.each(domUtils.getElementsByTagName(ut.table, "th"),
							function(node) {
								node.setAttribute("width", "");
							});
					var cells = ut.isFullRow() ? domUtils.getElementsByTagName(
		...
		UE.commands['averagedistributerow'] = {
			...
			execCommand : function(cmd) {
				...
				function getAverageHeight() {
					var averageHeight, rowNum, sumHeight = 0, tb = ut.table, tbAttr = getDefaultValue(
							me, tb), tdpadding = parseInt(domUtils
							.getComputedStyle(tb.getElementsByTagName('td')[0],
									"padding-top"));
					if (ut.isFullCol()) {
						var captionArr = domUtils.getElementsByTagName(tb,
								"caption"), thArr = domUtils
								.getElementsByTagName(tb, "th"), captionHeight, thHeight;
						...
						sumHeight = tb.offsetHeight - (captionHeight || 0)
								- (thHeight || 0);
						rowNum = thArr.length == 0 ? ut.rowsNum
								: (ut.rowsNum - 1);
					} else {
						var begin = ut.cellsRange.beginRowIndex, end = ut.cellsRange.endRowIndex, count = 0, trs = domUtils
								.getElementsByTagName(tb, "tr");
					...
					if (browser.ie && browser.version < 9) {
						...
						averageHeight = Math.ceil(sumHeight / rowNum)
								- tbAttr.tdBorder * 2 - tdpadding * 2;
					}
					return averageHeight;
				}
				function setAverageHeight(averageHeight) {
					var cells = ut.isFullCol() ? domUtils.getElementsByTagName(
							ut.table, "td") : ut.selectedTds;
					utils.each(cells, function(node) {
						if (node.rowSpan == 1) {
		...
		UE.commands['cellalignment'] = {
			...
			execCommand : function(cmd, data) {
				var me = this, ut = getUETableBySelected(me);
				if (!ut) {
					var start = me.selection.getStart(), cell = start
							&& domUtils.findParentByTagName(start, [ "td",
									"th", "caption" ], true);
		...
		UE.commands['tablealignment'] = {
			...
			execCommand : function(cmd, value) {
				var me = this, start = me.selection.getStart(), table = start
						&& domUtils.findParentByTagName(start, [ "table" ],
								true);
				if (table) {
		...
		UE.commands['edittable'] = {
			...
			execCommand : function(cmd, color) {
				var rng = this.selection.getRange(), table = domUtils
						.findParentByTagName(rng.startContainer, 'table');
				if (table) {
					var arr = domUtils.getElementsByTagName(table, "td")
							.concat(
									domUtils.getElementsByTagName(table, "th"),
									domUtils.getElementsByTagName(table,
											"caption"));
		...
		UE.commands['sorttable'] = {
			queryCommandState : function() {
				...
				if (!tableItems.cell)
					return -1;
				var table = tableItems.table, cells = table
			...
			execCommand : function(cmd, fn) {
				var me = this, range = me.selection.getRange(), bk = range
						.createBookmark(true), tableItems = getTableItemsByRange(me), cell = tableItems.cell, ut = getUETable(tableItems.table), cellInfo = ut
						.getCellInfo(cell);
		...
		function resetTdWidth(table, editor) {
			...
			table.setAttribute('width', getTableWidth(editor, true,
					getDefaultValue(editor, table)));
			setTimeout(function() {
				utils.each(tds, function(td) {
					(td.colSpan == 1)
		...
		function getTableWidth(editor, needIEHack, defaultValue) {
			...
			return body.offsetWidth
					- (needIEHack ? parseInt(domUtils.getComputedStyle(body,
							'margin-left'), 10) * 2 : 0)
...
//    }
	...
	UE.plugins['contextmenu'] = function () {
		var me = this,
			...
			items = me.options.contextMenu || [
				...
				{
					label:lang.clearnoborder,
					cmdName:'clearnoborder'
				},
				{
		...
		me.addListener( 'contextmenu', function ( type, evt ) {
			...
			if(me.queryCommandState('inserttable',me)!=-1){
				return;
			}
	...
	UE.plugins['basestyle'] = function() {
		...
		for ( var style in basestyles) {
			(function(cmd, tagNames) {
				me.commands[cmd] = {
					execCommand : function(cmdName) {
						var range = me.selection.getRange(), obj = getObj(this,
								tagNames);
						if (range.collapsed) {
							if (obj) {
								var tmpText = me.document.createTextNode('');
								range.insertNode(tmpText).removeInlineStyle(
										tagNames);
								range.setStartBefore(tmpText);
								domUtils.remove(tmpText);
							} else {
								var tmpNode = range.document
										.createElement(tagNames[0]);
								if (cmdName == 'superscript'
										|| cmdName == 'subscript') {
									tmpText = me.document.createTextNode('');
									range
											.insertNode(tmpText)
											.removeInlineStyle([ 'sub', 'sup' ])
											.setStartBefore(tmpText).collapse(
													true);
								}
							...
							if (cmdName == 'superscript'
									|| cmdName == 'subscript') {
								if (!obj
										|| obj.tagName.toLowerCase() != cmdName) {
									range.removeInlineStyle([ 'sub', 'sup' ]);
								}
							}
							obj ? range.removeInlineStyle(tagNames) : range
									.applyInlineStyle(tagNames[0]);
						}
						range.select();
					},
					queryCommandState : function() {
	...
	UE.plugins['elementpath'] = function() {
		...
		me.commands['elementpath'] = {
			...
			queryCommandValue : function() {
				var parents = [].concat(this.selection.getStartElementPath())
						.reverse(), names = [];
				tagNames = parents;
	...
	UE.plugins['formatmatch'] = function() {
		...
		function addList(type, evt) {
			...
			function addFormat(range) {
				...
				return range.applyInlineStyle(list[list.length - 1].tagName,
						null, list);
			}
			me.undoManger && me.undoManger.save();
			var range = me.selection.getRange(), imgT = target
					|| range.getClosedNode();
			if (img && imgT && imgT.tagName == 'IMG') {
				imgT.style.cssText += ';float:'
	...
	UE.plugins['searchreplace'] = function() {
		...
		me.commands['searchreplace'] = {
			execCommand : function(cmdName, opt) {
				var me = this, sel = me.selection, range, nativeRange, num = 0, opt = utils
						.extend(opt, {
							all : false,
							casesensitive : false,
							dir : 1
				...
				if (browser.ie) {
					...
					while (1) {
						...
						if (opt.all) {
							...
							opt.dir = 1;
							if (currentRange) {
								tmpRange.setEndPoint(
										opt.dir == -1 ? 'EndToStart'
												: 'StartToEnd', currentRange);
							} else {
								tmpRange.moveToElementText(me.document.body);
							}
						} else {
							tmpRange.setEndPoint(opt.dir == -1 ? 'EndToStart'
									: 'StartToEnd', nativeRange);
							if (opt.hasOwnProperty("replaceStr")) {
								tmpRange.setEndPoint(
										opt.dir == -1 ? 'StartToEnd'
												: 'EndToStart', nativeRange);
							}
						}
						nativeRange = tmpRange.duplicate();
						if (/^\/[^/]+\/\w*$/.test(opt.searchStr)) {
							var str = tmpRange.text, reg = new RegExp(
									opt.searchStr.replace(/^\/|\/\w*$/g, ''),
									'g' + (opt.casesensitive ? '' : 'i'));
							var match = str.match(reg);
							if (match && match.length) {
								searchStr = opt.dir < 0 ? match[match.length - 1]
										: match[0];
							} else {
								currentRange = null;
								return num;
						...
						if (!tmpRange.findText(searchStr, opt.dir,
								opt.casesensitive ? 4 : 0)) {
							currentRange = null;
					...
					while (1) {
						if (opt.all) {
							...
							if (opt.hasOwnProperty("replaceStr")) {
								nativeRange.collapse(opt.dir == 1 ? true
										: false);
							}
						}
						if (!first) {
						...
						if (/^\/[^/]+\/\w*$/.test(opt.searchStr)) {
							...
							if (opt.dir < 0) {
								...
								nativeRange.setEnd(me.body,
										me.body.childNodes.length);
							}
							var str = nativeRange + '', reg = new RegExp(
									opt.searchStr.replace(/^\/|\/\w*$/g, ''),
									'g' + (opt.casesensitive ? '' : 'i'));
							var match = str.match(reg);
							if (match && match.length) {
						...
						if (!w.find(searchStr, opt.casesensitive,
								opt.dir < 0 ? true : false)) {
							currentRange = null;
							nativeSel.removeAllRanges();
							return num;
						}
	...
	UE.plugins['customstyle'] = function() {
		...
		me.commands['customstyle'] = {
			execCommand : function(cmdName, obj) {
				var me = this, tagName = obj.tag, node = domUtils.findParent(
						me.selection.getStart(), function(node) {
				...
				if (node && node.getAttribute('label') == obj.label) {
					...
					if (range.collapsed) {
						...
						var common = domUtils.getCommonAncestor(bk.start,
								bk.end), nodes = domUtils.getElementsByTagName(
								common, tagName);
						...
						for (var i = 0, ni; ni = nodes[i++];) {
							if (ni.getAttribute('label') == obj.label) {
								var ps = domUtils.getPosition(ni, bk.start), pe = domUtils
										.getPosition(ni, bk.end);
								if ((ps & domUtils.POSITION_FOLLOWING || ps
										& domUtils.POSITION_CONTAINS)
										&& (pe & domUtils.POSITION_PRECEDING || pe
												& domUtils.POSITION_CONTAINS))
									if (dtd.$block[tagName]) {
										var fillNode = me.document
												.createElement('p');
										domUtils.moveChild(ni, fillNode);
										ni.parentNode
												.insertBefore(fillNode, ni);
									}
					...
					if (dtd.$block[tagName]) {
						...
						if (range.collapsed) {
							...
							range.insertNode(node).setStart(node, 0)
									.setCursor();
							return;
						}
						bk = range.createBookmark();
						range.applyInlineStyle(tagName, tmpObj).moveToBookmark(
								bk).select();
					}
				}
			},
			queryCommandValue : function() {
				var parent = domUtils.filterNodeList(this.selection
						.getStartElementPath(), function(node) {
					return node.getAttribute('label')
				});
		...
		me.addListener('keyup', function(type, evt) {
			...
			if (keyCode == 32 || keyCode == 13) {
				...
				if (range.collapsed) {
					...
					if (node && dtd.$block[node.tagName]
	...
	UE.plugins['catchremoteimage'] = function() {
		...
		me
				.addListener(
						...
						function() {
							var remoteImages = [];
							var imgs = domUtils.getElementsByTagName(
									me.document, "img");
								...
								catchremoteimage(
										remoteImages,
										{
											success : function(xhr) {
												...
												var srcUrls = info.srcUrl
														.split(separater), urls = info.url
														.split(separater);
												for (var i = 0, ci; ci = imgs[i++];) {
													var src = ci
															.getAttribute("_src")
													...
													for (var j = 0, cj; cj = srcUrls[j++];) {
														...
														if (src == cj
																&& url != "error") {
															var newSrc = me.options.catcherPath
																	+ url;
															domUtils
																	.setAttributes(
																			ci,
												...
												me
														.fireEvent('catchremotesuccess')
											},
											error : function() {
												me
														.fireEvent("catchremoteerror");
											}
										});
							}
						});
		...
		me.commands['snapscreen'] = {
			execCommand : function() {
				...
				try {
					...
					editorOptions.snapscreenServerUrl = editorOptions.snapscreenServerUrl
							.split(editorOptions.snapscreenHost);
					editorOptions.snapscreenServerUrl = editorOptions.snapscreenServerUrl[1]
							|| editorOptions.snapscreenServerUrl[0];
					if (editorOptions.snapscreenServerUrl.indexOf(":" + port) === 0) {
						editorOptions.snapscreenServerUrl = editorOptions.snapscreenServerUrl
								.substring(port.length + 1);
					}
					var ret = snapplugin.saveSnapshot(
							editorOptions.snapscreenHost,
							editorOptions.snapscreenServerUrl, port);
					onSuccess(ret);
				} catch (e) {
	...
	UE.plugins['webapp'] = function() {
		...
		function createInsertStr(obj, toIframe, addParagraph) {
			return !toIframe ? (addParagraph ? '<p>' : '')
					+ '<img title="'
					+ obj.title
					+ '" width="'
					+ obj.width
					...
					+ obj.url + '" />' + (addParagraph ? '</p>' : '')
					: '<iframe class="edui-faked-webapp" title="' + obj.title
							+ '" width="' + obj.width + '" height="'
							+ obj.height
							+ '"  scrolling="no" frameborder="0" src="'
							+ obj.url + '" logo_url = ' + obj.logo
							+ '></iframe>';
		}
		function switchImgAndIframe(img2frame) {
			var tmpdiv, nodes = domUtils.getElementsByTagName(me.document,
					!img2frame ? "iframe" : "img");
			for (var i = 0, node; node = nodes[i++];) {
				...
				tmpdiv.innerHTML = createInsertStr(img2frame ? {
					...
					height : node.height,
					title : node.title,
					logo : node.style.backgroundImage.replace("url(", "")
							.replace(")", "")
				} : {
		...
		me.addListener('aftergetcontent',
				function(cmdName) {
					if (cmdName == 'aftergetcontent'
							&& me.queryCommandState('source')) {
						return;
					}
					switchImgAndIframe(false);
				});
	...
	UE.plugins['template'] = function() {
		...
		this.addListener("click", function(type, evt) {
			var el = evt.target || evt.srcElement, range = this.selection
					.getRange();
		...
		this.addListener("keydown", function(type, evt) {
			...
			if (!range.collapsed) {
				if (!evt.ctrlKey && !evt.metaKey && !evt.shiftKey
						&& !evt.altKey) {
					var tnode = domUtils.findParent(range.startContainer,
							function(node) {
								if (node.className
										&& domUtils.hasClass(node, "ue_t")) {
									return node;
								}
							}, true);
					if (tnode) {
	...
	UE.plugins['music'] = function() {
		...
		function creatInsertStr(url, width, height, align, toEmbed,
				addParagraph) {
			return !toEmbed ? (addParagraph ? ('<p '
					+ (align != "none" ? (align == "center" ? ' style="text-align:center;" '
							: ' style="float:"' + align)
							: '') + '>')
					: '')
					+ '<img align="'
					...
					+ me.options.langPath
					+ me.options.lang
					+ '/images/music.png" />' + (addParagraph ? '</p>' : '')
		...
		function switchImgAndEmbed(img2embed) {
			...
			for (var i = 0, node; node = nodes[i++];) {
				...
				align = align == 'none' ? (node.getAttribute('align') || '')
						: align;
				tmpdiv.innerHTML = creatInsertStr(img2embed ? node
						.getAttribute("_url") : node.getAttribute("src"),
						node.width, node.height, align, img2embed);
				node.parentNode.replaceChild(tmpdiv.firstChild, node);
			}
		}
		me.addListener("beforegetcontent", function() {
		...
		me.commands["music"] = {
			execCommand : function(cmd, musicObj) {
				var me = this, str = creatInsertStr(musicObj.url,
						musicObj.width || 400, musicObj.height || 95, "none",
						false, true);
				me.execCommand("inserthtml", str);
			},
			queryCommandState : function() {
				var me = this, img = me.selection.getRange().getClosedNode(), flag = img
						&& (img.className == "edui-faked-music");
				return flag ? 1 : 0;
			}
		};
	};
	...
	(function() {
		...
		var uiUtils = baidu.editor.ui.uiUtils = {
			uid : function(obj) {
				return (obj ? obj[uidMagic] || (obj[uidMagic] = ++uidCount)
						: ++uidCount);
			},
			...
			getViewportElement : function() {
				return (browser.ie && browser.quirks) ? document.body
			...
			contains : function(elA, elB) {
				return elA
						&& elB
						&& (elA === elB ? false : (elA.contains ? elA
								.contains(elB) : elA
	...
	(function() {
		...
		UIBase.prototype = {
			...
			initUIBase : function() {
				this._globalKey = utils
						.unhtml(uiUtils.setGlobal(this.id, this));
			},
	...
	(function() {
		var utils = baidu.editor.utils, UIBase = baidu.editor.ui.UIBase, Separator = baidu.editor.ui.Separator = function(
				options) {
	...
	(function() {
		var utils = baidu.editor.utils, uiUtils = baidu.editor.ui.uiUtils, domUtils = baidu.editor.dom.domUtils, UIBase = baidu.editor.ui.UIBase, Popup = baidu.editor.ui.Popup = function(
				options) {
			this.initOptions(options);
			this.initPopup();
		};
			...
			for (var i = 0; i < allPopups.length; i++) {
				...
				if (!pop.isHidden()) {
					if (pop.queryAutoHide(el) !== false) {
						if (evt && /scroll/ig.test(evt.type)
		...
		Popup.prototype = {
			...
			postRender : function() {
				...
				if (this.captureWheel && !this.captured) {
					this.captured = true;
					var winHeight = (document.documentElement.clientHeight || document.body.clientHeight) - 80, _height = this
							.getDom().offsetHeight, _top = domUtils
							.getXY(this.combox.getDom()).y, content = this
							.getDom('content'), me = this;
					...
					if (window.XMLHttpRequest) {
						domUtils
								.on(
										content,
										('onmousewheel' in document.body) ? 'mousewheel'
												: 'DOMMouseScroll',
										function(e) {
											if (e.preventDefault) {
												e.preventDefault();
			...
			showAnchorRect : function(rect, hoz, adj) {
				...
				if (hoz) {
					sideLeft = this.canSideLeft
							&& (rect.right + popSize.width > vpRect.right && rect.left > popSize.width);
					sideUp = this.canSideUp
							&& (rect.top + popSize.height > vpRect.bottom && rect.bottom > popSize.height);
	...
	(function() {
		var utils = baidu.editor.utils, UIBase = baidu.editor.ui.UIBase, ColorPicker = baidu.editor.ui.ColorPicker = function(
				options) {
			this.initOptions(options);
			this.noColorText = this.noColorText
					|| this.editor.getLang("clearColor");
			this.initUIBase();
		};
		...
		function genColorPicker(noColorText, editor) {
			...
			for (var i = 0; i < COLORS.length; i++) {
				if (i && i % 10 === 0) {
					html += '</tr>'
							+ (i == 60 ? '<tr style="border-bottom: 1px solid #ddd;font-size: 13px;line-height: 25px;color:#39C;"><td colspan="10">'
				...
				html += i < 70 ? '<td style="padding: 0 2px;"><a hidefocus title="'
						+ COLORS[i]
						...
						+ COLORS[i]
						+ ';border:solid #ccc;'
						+ (i < 10 || i >= 60 ? 'border-width:1px;' : i >= 10
	...
	(function() {
		...
		TablePicker.prototype = {
			...
			render : function(holder) {
				...
				this.getDom('label').innerHTML = '0'
						+ this.editor.getLang("t_row") + ' x 0'
			...
			_track : function(numCols, numRows) {
				...
				label.innerHTML = numCols + this.editor.getLang("t_col")
						+ ' x ' + numRows + this.editor.getLang("t_row");
	...
	(function() {
		...
		baidu.editor.ui.Stateful = {
			...
			hasState : function(state) {
				return domUtils.hasClass(this.getStateDom(), 'edui-state-'
						+ state);
			},
			...
			removeState : function(state) {
				if (this.hasState(state)) {
					domUtils.removeClasses(this.getStateDom(), [ 'edui-state-'
							+ state ]);
				}
			},
	...
	(function() {
		var utils = baidu.editor.utils, UIBase = baidu.editor.ui.UIBase, Stateful = baidu.editor.ui.Stateful, Button = baidu.editor.ui.Button = function(
				options) {
			this.initOptions(options);
			this.initButton();
		};
	...
	(function() {
		var utils = baidu.editor.utils, uiUtils = baidu.editor.ui.uiUtils, domUtils = baidu.editor.dom.domUtils, UIBase = baidu.editor.ui.UIBase, Stateful = baidu.editor.ui.Stateful, SplitButton = baidu.editor.ui.SplitButton = function(
				options) {
			this.initOptions(options);
	...
	(function() {
		var utils = baidu.editor.utils, uiUtils = baidu.editor.ui.uiUtils, ColorPicker = baidu.editor.ui.ColorPicker, Popup = baidu.editor.ui.Popup, SplitButton = baidu.editor.ui.SplitButton, ColorButton = baidu.editor.ui.ColorButton = function(
				options) {
			this.initOptions(options);
		...
		ColorButton.prototype = {
			...
			postRender : function() {
				...
				this.getDom('button_body').appendChild(
						uiUtils.createElementByHtml('<div id="' + this.id
	...
	(function() {
		...
		AutoTypeSetPicker.prototype = {
			...
			getHtmlTpl : function() {
				var me = this.editor, opt = me.options.autotypeset, lang = me
						.getLang("autoTypeSet");
				var textAlignInputName = 'textAlignValue' + me.uid, imageBlockInputName = 'imageBlockLineValue'
						+ me.uid;
				return '<div id="##" class="edui-autotypesetpicker %%">'
						...
						+ lang.removeHtml
						+ '</td></tr>'
						+ '<tr><td nowrap colspan="4"><input type="checkbox" name="pasteFilter" '
						+ (opt["pasteFilter"] ? "checked" : "")
						+ '>'
						+ lang.pasteFilter
						+ '</td></tr>'
						+ '<tr><td nowrap colspan="4" align="right"><button >'
						+ lang.run
						+ '</button></td></tr>'
						+ '</table>'
						+ '</div>' + '</div>';
	...
	(function() {
		var utils = baidu.editor.utils, Popup = baidu.editor.ui.Popup, AutoTypeSetPicker = baidu.editor.ui.AutoTypeSetPicker, SplitButton = baidu.editor.ui.SplitButton, AutoTypeSetButton = baidu.editor.ui.AutoTypeSetButton = function(
				options) {
			this.initOptions(options);
			this.initAutoTypeSetButton();
		};
		function getPara(me) {
			var opt = me.editor.options.autotypeset, cont = me.getDom(), editorId = me.editor.uid, inputType = null, attrName = null, ipts = domUtils
					.getElementsByTagName(cont, "input");
			for (var i = ipts.length - 1, ipt; ipt = ipts[i--];) {
				...
				if (inputType == "checkbox") {
					...
					if (ipt.checked) {
						var attrValue = document.getElementById(attrName
								+ "Value" + editorId);
						if (attrValue) {
							if (/input/ig.test(attrValue.tagName)) {
								opt[attrName] = attrValue.value;
							} else {
								var iptChilds = attrValue
	...
	(function() {
		...
		CellAlignPicker.prototype = {
			initSelected : function() {
				...
				if (this.selected) {
					this.selectedIndex = status.valign[this.selected.valign]
							* status.count + status.align[this.selected.align];
				}
			},
			...
			_onClick : function(evt) {
				...
				if (/icon/.test(target.className)) {
					this.items[target.parentNode.getAttribute("index")]
							.onclick();
					Popup.postHide(evt);
				}
			},
	...
	(function() {
		...
		PastePicker.prototype = {
			...
			_onClick : function(cur) {
				var node = domUtils.getNextDomNode(cur), screenHt = uiUtils
						.getViewportRect().height, subPop = uiUtils
						.getClientRect(node);
				if ((subPop.top + subPop.height) > screenHt)
					node.style.top = (-subPop.height - cur.offsetHeight) + "px";
				else
					node.style.top = "";
				if (/hidden/ig.test(domUtils.getComputedStyle(node,
						"visibility"))) {
	...
	(function() {
		var utils = baidu.editor.utils, uiUtils = baidu.editor.ui.uiUtils, UIBase = baidu.editor.ui.UIBase, Toolbar = baidu.editor.ui.Toolbar = function(
				options) {
			this.initOptions(options);
			this.initToolbar();
		};
	...
	(function() {
		var utils = baidu.editor.utils, domUtils = baidu.editor.dom.domUtils, uiUtils = baidu.editor.ui.uiUtils, UIBase = baidu.editor.ui.UIBase, Popup = baidu.editor.ui.Popup, Stateful = baidu.editor.ui.Stateful, CellAlignPicker = baidu.editor.ui.CellAlignPicker, Menu = baidu.editor.ui.Menu = function(
				options) {
			this.initOptions(options);
			this.initMenu();
		};
		...
		var MenuItem = baidu.editor.ui.MenuItem = function(options) {
			...
			if (this.subMenu && !(this.subMenu instanceof Menu)) {
				if (options.className
						&& options.className.indexOf("aligntd") != -1) {
					var me = this;
					this.subMenu.selected = this.editor
							.queryCommandValue('cellalignment');
		...
		MenuItem.prototype = {
			...
			postRender : function() {
				...
				if (this.subMenu) {
					...
					this.ownerMenu.addListener('submenuover', function(t,
							subMenu) {
						if (subMenu !== me) {
							me.delayHideSubMenu();
						}
	...
	(function() {
		var utils = baidu.editor.utils, uiUtils = baidu.editor.ui.uiUtils, Menu = baidu.editor.ui.Menu, SplitButton = baidu.editor.ui.SplitButton, Combox = baidu.editor.ui.Combox = function(
				options) {
			this.initOptions(options);
			this.initCombox();
		};
	...
	(function() {
		var utils = baidu.editor.utils, domUtils = baidu.editor.dom.domUtils, uiUtils = baidu.editor.ui.uiUtils, Mask = baidu.editor.ui.Mask, UIBase = baidu.editor.ui.UIBase, Button = baidu.editor.ui.Button, Dialog = baidu.editor.ui.Dialog = function(
				options) {
		...
		Dialog.prototype = {
			...
			showAtCenter:function() {
                this.getDom().style.display = "";
                var vpRect = uiUtils.getViewportRect();
                var editorCon = this.editor.container;
                var popSize = this.fitSize();
                var titleHeight = this.getDom("titlebar").offsetHeight | 0;
                var left = vpRect.width / 2 - popSize.width / 2;
                //var top = vpRect.height / 2 - (popSize.height - titleHeight) / 2 - titleHeight;
                // 让dialog窗口放在editor上面
                var top = editorCon.offsetTop;
                var popEl = this.getDom();
                this.safeSetOffset({
			...
			getContentHtml : function() {
				...
				if (typeof this.content == 'string') {
					...
					var allowAttribute = (this.className == "edui-for-recording");
					contentHtml = '<span id="'
							+ this.id
							+ '_contmask" class="dialogcontmask"></span><iframe id="'
							+ this.id
							+ '_iframe" class="%%-iframe" height="100%" width="100%" frameborder="0" '+ (allowAttribute?'allow="camera;microphone"':'')+ ' src="'
			...
			postRender : function() {
				...
				domUtils.on(window, 'resize', function() {
					setTimeout(function() {
						if (!me.isHidden()) {
							me
									.safeSetOffset(uiUtils.getClientRect(me
											.getDom()));
						}
					});
				});
				if (this.holdScroll) {
					if (!me.iframeUrl) {
						domUtils.on(document.getElementById(me.id + "_iframe"),
								!browser.gecko ? "mousewheel"
				...
				if (this.draggable) {
					...
					uiUtils.startDrag(evt, {
						...
						ondragstop : function() {
							...
							domUtils.removeClasses(me.getDom(),
									[ 'edui-state-centered' ]);
							me.dragMask.hide();
						}
					});
			...
			_show : function() {
				if (this._hidden) {
					...
					this._hidden = false;
					this.fireEvent('show');
					baidu.editor.ui.uiUtils.getFixedLayer().style.zIndex = this
							.getDom().style.zIndex - 4;
				}
			},
			isHidden : function() {
				return this._hidden;
			...
			close : function(ok) {
				if(this.editor.selection.getRange && this.editor.selection.getRange().collapsed){
					this.editor.body && this.editor.body.focus();
				}
	...
	(function() {
		...
		var iframeUrlMap = {
			...
			'insertimage' : '~/dialogs/image/imageNew.html?v=2023-1117-1120' + '#'
					+ window.UEDITOR_CONFIG.DIYDOMAIN,
			'link' : '~/dialogs/link/link.html' + '#'
					+ window.UEDITOR_CONFIG.DIYDOMAIN,
			'spechars' : '~/dialogs/spechars/spechars.html' + '#' + window.UEDITOR_CONFIG.DIYDOMAIN,
			...
			'attachment_new' : '~/dialogs/attachment_new/attachment.html?v=2023-1117-2033' + '#'
					+ window.UEDITOR_CONFIG.DIYDOMAIN,
			'symbol' : '~/dialogs/symbol/symbol.html' + '#'
					+ window.UEDITOR_CONFIG.DIYDOMAIN,
			'tpupload' : '~/dialogs/tpupload/tpupload.html?v=2024-0425-1840' + '#'
				+ window.UEDITOR_CONFIG.DIYDOMAIN,
			'mathml' : '~/dialogs/mathml/mathml.html?v=2024-0119-2100' + '#'
					+ window.UEDITOR_CONFIG.DIYDOMAIN,
		    'edrawmath' : '~/dialogs/edrawmath/edrawmath.html?v=2024-0710-1530' + '#'
                 	+ window.UEDITOR_CONFIG.DIYDOMAIN,		
           'calculator' : '~/dialogs/calculator/index.html?v=2023-0920-1700' + '#'
                 	+ window.UEDITOR_CONFIG.DIYDOMAIN,		
           'latex' : '/ananas/common-modules/latex/html/mooc1-index.html?mooc2=1&v=2024-0925-2300' + '#'
 					+ window.UEDITOR_CONFIG.DIYDOMAIN,
           'drawingboard' : '~/dialogs/drawingboard/index.html?v=2023-0718-1700' + '#'
                 	+ window.UEDITOR_CONFIG.DIYDOMAIN,		
			'insertframe' : '~/dialogs/insertframe/insertframe.html',
			'edittip' : '~/dialogs/table/edittip.html',
			'edittable' : '~/dialogs/table/edittable.html' + '#' + window.UEDITOR_CONFIG.DIYDOMAIN,
			...
			'recording' : '/ananas/ueditor/dialogs/recoder/recording-workexam.html?v=2022-1017-1648',
			'audio' : '~/dialogs/audio/attachmentNew.html?v=2023-1117-1120' + '#' + window.UEDITOR_CONFIG.DIYDOMAIN
		};
		var btnCmds = [ 'undo', 'redo', 'formatmatch', 'bold', 'italic',
				'underline', 'fontborder', 'touppercase', 'tolowercase',
				'strikethrough', 'subscript', 'superscript', 'source',
				'indent', 'outdent', 'blockquote', 'pasteplain', 'pagebreak',
				'selectall', 'print', 'preview', 'horizontal', 'removeformat',
				'time', 'date', 'unlink', 'insertparagraphbeforetable',
				'insertrow', 'insertcol', 'mergeright', 'mergedown',
				'deleterow', 'deletecol', 'splittorows', 'splittocols',
				'splittocells', 'mergecells', 'deletetable', 'attachment_new',
				'tpupload', 'symbol', 'mathml','latex','edrawmath','calculator','drawingboard','insertcodedefined'];
		...
		for ( var p in typeset) {
			(function(cmd, val) {
				for (var i = 0, ci; ci = val[i++];) {
					(function(cmd2) {
						editorui[cmd.replace('float', '') + cmd2] = function(
								editor) {
							var ui = new editorui.Button({
								className : 'edui-for-'
										+ cmd.replace('float', '') + cmd2,
								title : editor.options.labelMap[cmd.replace(
										'float', '')
										+ cmd2]
							...
							editor.addListener('selectionchange', function(
								...
								ui
										.setDisabled(editor
												.queryCommandState(cmd) == -1);
								ui
										.setChecked(editor
												.queryCommandValue(cmd) == cmd2
												&& !uiReady);
							});
							return ui;
						};
		...
		var dialogBtns = {
			...
			ok : [ 'attachment', 'anchor', 'link', 'insertimage', 'map',
					'gmap', 'insertframe', 'wordimage', 'insertvideo',
					'insertframe', 'edittip', 'edittable', 'edittd', 'scrawl',
					'template', 'music', 'background', 'attachment_new', 'tpupload',
					'symbol', 'mathml','recording','audio','edrawmath','latex','calculator','drawingboard']
		};
		var dialogNoBottomBtns=['recording','attachment_new', 'insertimage', 'audio', 'tpupload','edrawmath','calculator','drawingboard','latex'];//设置对话框不显示底部按钮
					...
					(function(cmd) {
						editorui[cmd] = function(editor, iframeUrl, title) {
							...
							if (iframeUrl) {
								...
								}, type == 'ok' && dialogNoBottomBtns.join(",").indexOf(cmd) == -1  ? {
									buttons : [ {
										...
										onclick : function() {
											if (dialog) {
												...
												case "scrawl":
													...
													break;
												default:
													var forbidDownload = editor.getOpt('forbidDownload');
													if(cmd == "recording" && forbidDownload === 'true') {
														alert("现在上传人数过多，请稍后尝试");
														return;
													}
		...
		editorui.snapscreen = function(editor, iframeUrl, title) {
			title = editor.options.labelMap['snapscreen']
		...
		editorui.insertcode = function(editor, list, title) {
			...
			title = editor.options.labelMap['insertcode']
		...
		editorui.fontfamily = function(editor, list, title) {
			...
			title = editor.options.labelMap['fontfamily']
		...
		editorui.fontsize = function(editor, list, title) {
			title = editor.options.labelMap['fontsize']
		...
		editorui.paragraph = function(editor, list, title) {
			title = editor.options.labelMap['paragraph']
		...
		editorui.customstyle = function(editor) {
			var list = editor.options['customstyle'] || [], title = editor.options.labelMap['customstyle']
		...
		editorui.inserttable = function(editor, iframeUrl, title) {
			title = editor.options.labelMap['inserttable']
				...
				editorui['rowspacing' + cmd] = function(editor) {
					...
					for (var i = 0, ci, items = []; ci = val[i++];) {
						items.push({
							...
							theme : editor.options.theme,
							onclick : function() {
								editor.execCommand("rowspacing", this.value,
										cmd);
							}
						})
					}
					...
					editor.addListener('selectionchange',
							function() {
								...
								if (state == -1) {
									...
									ui.setDisabled(false);
									var value = editor.queryCommandValue(
											'rowspacing', cmd);
									value
											&& ui.setValue((value + '')
													.replace(/%/, ''));
		...
		editorui.fullscreen = function(editor, title) {
			title = editor.options.labelMap['fullscreen']
	...
	(function() {
		...
		EditorUI.prototype = {
			...
			initEditorUI : function() {
						...
						.addListener(
								...
								function() {
									editor.getDialog = function(name) {
										return editor.ui._dialogs[name
												+ "Dialog"];
									};
									domUtils.on(editor.window, 'scroll',
											function(evt) {
												baidu.editor.ui.Popup
														.postHide(evt);
											});
									editor.ui._actualFrameWidth = editor.options.initialFrameWidth;
									if (editor.options.elementPathEnabled) {
										editor.ui.getDom('elementpath').innerHTML = '<div class="edui-editor-breadcrumb">'
												+ editor
														.getLang("elementPathTip")
												+ ':</div>';
									}
									if (editor.options.wordCount) {
										function countFn() {
											setCount(editor, me);
											domUtils.un(editor.document,
													"click", arguments.callee);
										}
										domUtils.on(editor.document, "click",
												countFn);


                                        // 初始化填充
										//var initCount = editor.getContentLength(true);
										var initCount = editor.pureWordCount();
										var max = editor.options.maximumWords;
										var min = editor.options.minimumWords;
										var maxMsg = editor.options.wordMaxManage;
										var minMsg = editor.options.wordMinManage;
										var nmsg = editor.options.wordCountNum;
										if (max == 0 && min == 0) {
											if (initCount > 0) {
												editor.ui.getDom('wordcount').innerHTML = nmsg.replace("{#count}", initCount);
											} else {
											...
											if (initCount > 0) {
												...
												if (max > 0 && min == 0) {
													editor.ui.getDom('wordcount').innerHTML = maxMsg.replace("{#maxword}", max);
												}else if (max == 0 && min > 0) {
													editor.ui.getDom('wordcount').innerHTML = minMsg.replace("{#minword}", min);
												} else {
													editor.ui.getDom('wordcount').innerHTML = minMsg.replace("{#minword}", min)+','+ maxMsg.replace("{#maxword}", max);
												}
											}
										}
									...
									if (!editor.selection.isFocus())
										return;
									editor.fireEvent('selectionchange', false,
											true);
								});
												...
												if (pastePop
													...
													if (pastePop.isHidden()) {
														...
														var tmp = getDomNode(
																...
																'previousSibling');
														pastePop
																.showAnchor(tmp.nodeType == 3 ? tmp.parentNode
																		: tmp);
														domUtils.remove(span);
													} else {
														pastePop.show();
				...
				function setCount(editor, ui) {
					editor.setOpt({
						...
						maximumWords : 10000,
						minimumWords : 0,
						wordCountMsg : editor.options.wordCountMsg
								|| editor.getLang("wordCountMsg"),
						wordOverFlowMsg : editor.options.wordOverFlowMsg
								|| editor.getLang("wordOverFlowMsg"),
						wordCountNum : editor.options.wordCountNum
							|| editor.getLang("wordCountNum")
					});
					var opt = editor.options, max = opt.maximumWords,min = opt.minimumWords, msg = opt.wordCountMsg, errMsg = opt.wordOverFlowMsg,wordNeedFlowMsg = opt.wordNeedFlowMsg, countDom = ui
						.getDom('wordcount'), curCountMsg = opt.wordCountNum,curCountMsg = opt.wordCountNum, maxMsg = opt.wordMaxManage,minMsg = opt.wordMinManage;;
					if (!opt.wordCount) {
						return;
					}

					//var count = editor.getContentLength(true);
					var count =  editor.pureWordCount();
					if (max == 0 && min == 0) {
						if (count == 0) {
							countDom.innerHTML = "";
						} else {
							countDom.innerHTML = curCountMsg.replace("{#count}", count);
						}
					} else {
						if (count == 0) {
							if (max > 0 && min == 0) {
								countDom.innerHTML  = maxMsg.replace("{#maxword}", max);
							}else if (max == 0 && min > 0) {
								countDom.innerHTML  = minMsg.replace("{#minword}", min);
							} else {
								countDom.innerHTML  = minMsg.replace("{#minword}", min)+','+ maxMsg.replace("{#maxword}", max);
							}
						}else if((count >= min && count <= max) || (count >= min && max == 0)||(min == 0 && count <= max)) {
							countDom.innerHTML = curCountMsg.replace("{#count}", count);
						} else if(count < min){
							var needs = min - count;
							countDom.innerHTML = wordNeedFlowMsg.replace("{#needs}", needs);
						}else if (count > max) {
							//countDom.innerHTML = errMsg;
							var overs = count - max;
							countDom.innerHTML = errMsg.replace("{#overs}", overs);
							editor.fireEvent("wordcountoverflow");

							// 超出最大限制alert弹层改为可配置[wordCountTimer= true / false]
							var wordCountTimer = editor.options.wordCountTimer;
							if(wordCountTimer) {
								setTimeout(function() {
									if (editor.getContentLength(true) - max > 20) {//粘贴大量字符情况
										alert("输入字符数量已经超出最大允许值，将清空处理，请重新输入！");
										editor.setContent("");
									} else if (editor.getContentLength(true) - max > 0) {//手动输入字符情况
										alert("输入字符数量已经超出最大允许值，将截取处理，请重新输入！");
				...
				editor
						.addListener(
								...
								function() {
									if (editor.options.elementPathEnabled) {
										me[(editor
												.queryCommandState('elementpath') == -1 ? 'dis'
												: 'en')
												+ 'ableElementPath']()
									}
									if (editor.options.scaleEnabled) {
										me[(editor.queryCommandState('scale') == -1 ? 'dis'
												: 'en')
												+ 'ableScale']();
									}
								});
				var popup = new baidu.editor.ui.Popup({
					...
					_onImgEditButtonClick : function(name) {
						this.hide();
						editor.ui._dialogs[name]
								&& editor.ui._dialogs[name].open();
					},
					_onImgSetFloat : function(value) {
						this.hide();
						editor.execCommand("imagefloat", value);
					},
					_onImgTwiceReview : function(value) {
						this.hide();
						editor.execCommand("review", value);
					},
					...
					queryAutoHide : function(el) {
						if (el && el.ownerDocument == editor.document) {
							if (el.tagName.toLowerCase() == 'img'
									|| domUtils.findParentByTagName(el, 'a',
											true)) {
				...
				if (editor.options.imagePopup) {
					editor
							.addListener(
									...
									function(t, evt) {
										...
										var el = evt.target || evt.srcElement;
										if (editor.ui._dialogs.insertframeDialog
												&& /iframe/ig.test(el.tagName)) {
											var html = popup
													.formatHtml('<nobr>'
															+ editor
																	.getLang("property")
					...
					editor
							.addListener(
									'selectionchange',
									function(t, causeByUi) {
										if (!causeByUi)
											return;
										var html = '', str = "", img = editor.selection
												.getRange().getClosedNode(), dialogs = editor.ui._dialogs;
										if (img && img.tagName == 'IMG') {
											...
											if (img.className.indexOf("ans-formula-moudle") != -1) {
												if(editor.options && editor.options.latexMode == 1){
													 dialogName = "latexDialog"
												 }else{
												     dialogName = "mathmlDialog"
												 }
											}
											
											if (img.className.indexOf("ans-latex-moudle") != -1) {
												dialogName = "latexDialog"
											}
											
											if (img.className.indexOf("ans-edrawmath-moudle") != -1) {
												dialogName = "edrawmathDialog"
	                                         }
											...
											str = '<nobr>'
												...
												+ '</span>&nbsp;&nbsp;';
											if (typeof ueReview !== "undefined" && ueReview) {
												str += '<span onclick=$$._onImgTwiceReview() class="edui-clickable">'
														+ editor.getLang("review")
														+ '</span>&nbsp;&nbsp;';
											}
											str += '</nobr>';
					...
					for (var j = 0; j < toolbar.length; j++) {
						...
						if (typeof toolbarItem == 'string') {
							...
							if (baidu.editor.ui[toolbarItem]) {
								toolbarItemUi = new baidu.editor.ui[toolbarItem](
										editor);
							}
							if (toolbarItem == 'fullscreen') {
								if (toolbarUis && toolbarUis[0]) {
									toolbarUis[0].items.splice(0, 0,
											toolbarItemUi);
								} else {
									toolbarItemUi
											&& toolbarUi.items.splice(0, 0,
													toolbarItemUi);
								}
								continue;
							}
				...
				if (this._fullscreen != fullscreen) {
					...
					if (fullscreen) {
						while (container.tagName != "BODY") {
							var position = baidu.editor.dom.domUtils
			...
			_updateFullScreen : function() {
				if (this._fullscreen) {
					...
					this.editor.setHeight(vpRect.height
			...
			_updateElementPath : function() {
				...
				if (this.elementPathEnabled
					...
					bottom.innerHTML = '<div class="edui-editor-breadcrumb" onmousedown="return false;">'
			...
			_scale : function() {
				var doc = document, editor = this.editor, editorHolder = editor.container, editorDocument = editor.document, toolbarBox = this
						.getDom("toolbarbox"), bottombar = this
						.getDom("bottombar"), scale = this.getDom("scale"), scalelayer = this
						.getDom("scalelayer");
				...
				function down() {
					...
					if (!minEditorHeight) {
						minEditorHeight = editor.options.minFrameHeight
								+ toolbarBox.offsetHeight
								+ bottombar.offsetHeight;
					}
					scalelayer.style.cssText = "position:absolute;left:0;display:;top:0;background-color:#41ABFF;opacity:0.4;filter: Alpha(opacity=40);width:"
				...
				var me = this;
				this.editor.addListener('fullscreenchanged', function(e,
						fullScreen) {
					if (fullScreen) {
						me.disableScale();
					} else {
						if (me.editor.options.scaleEnabled) {
							...
							me.editor.body.style.height = Math.max(domUtils
									.getXY(tmpNode).y,
				...
				function move(event) {
					...
					pageX = e.pageX
							|| (doc.documentElement.scrollLeft + e.clientX);
					pageY = e.pageY
							|| (doc.documentElement.scrollTop + e.clientY);
				...
				function up() {
					if (isMouseMove) {
						...
						editorHolder.style.width = editor.ui._actualFrameWidth
								+ 'px';
						editor.setHeight(scalelayer.offsetHeight
								- bottombar.offsetHeight
								- toolbarBox.offsetHeight - 2);
					}
					if (scalelayer) {
			...
			mapUrl : function(url) {
				return url ? url.replace('~/',
						this.editor.options.UEDITOR_HOME_URL || '') : ''
			},
		...
		UE.ui.Editor = function(options) {
			...
			utils.loadFile(document, {
				href : editor.options.themePath + editor.options.theme
						+ "/css/ueditor.css?v=2024-0930-1428",
				tag : "link",
				type : "text/css",
				rel : "stylesheet"
			});
			...
			editor.render = function(holder) {
				...
				utils
						.domReady(function() {
							editor.langIsReady ? renderUI() : editor
									.addListener("langReady", renderUI);
							function renderUI() {
								editor.setOpt({
								...
								if (holder) {
									...
									holder
											...
											&& (editor.options.textarea = holder
													.getAttribute('name'));
									if (holder
											&& /script|textarea/ig
													.test(holder.tagName)) {
										var newDiv = document
												.createElement('div');
										holder.parentNode.insertBefore(newDiv,
												holder);
										var cont = holder.value
												|| holder.innerHTML;
								...
								domUtils.addClass(holder, "edui-"
										+ editor.options.theme);
								editor.ui.render(holder);
								var opt = editor.options;
								editor.container = editor.ui.getDom();
								var parents = domUtils
										.findParents(holder, true);
								var displays = [];
								for (var i = 0, ci; ci = parents[i]; i++) {
								...
								editor.container.style.width = opt.initialFrameWidth
										+ (/%$/.test(opt.initialFrameWidth) ? ''
												: 'px');
								editor.container.style.zIndex = opt.zIndex;
								oldRender.call(editor, editor.ui
										.getDom('iframeholder'));
							}
						})
			};
	...
	(function() {
		var utils = baidu.editor.utils, Popup = baidu.editor.ui.Popup, SplitButton = baidu.editor.ui.SplitButton, MultiMenuPop = baidu.editor.ui.MultiMenuPop = function(
				options) {
			this.initOptions(options);
			this.initMultiMenu();
		};
	...
	(function() {
		...
		ShortCutMenu.prototype = {
						...
						.on(
								...
								function(e) {
									if (me.isHidden === false) {
										if (me.getSubMenuMark()
												|| me.eventType == "contextmenu")
											return;
										var flag = true, el = me.getDom(), wt = el.offsetWidth, ht = el.offsetHeight, distanceX = wt
												/ 2 + me.SPACE, distanceY = ht / 2, x = Math
												.abs(e.screenX - me.left), y = Math
												.abs(e.screenY - me.top);
										clearTimeout(timeID);
				...
				if (browser.chrome) {
					domUtils
							.on(
									...
									"mouseout",
									function(e) {
										var relatedTgt = e.relatedTarget
												|| e.toElement;
										if (relatedTgt == null
												|| relatedTgt.tagName == "HTML") {
											me.hide();
										}
									});
				}
			...
			setOpacity : function(el, value) {
				if (browser.ie && browser.version < 9) {
					el.style.filter = "alpha(opacity = " + parseFloat(value)
							* 100 + ");"
				} else {
			...
			getSubMenuMark : function() {
				...
				var list = domUtils.getElementsByTagName(layerEle, "div",
						function(node) {
							return domUtils.hasClass(node,
									"edui-shortcutsubmenu edui-popup")
						});
				for (var i = 0, node; node = list[i++];) {
			...
			show : function(e, hasContextmenu) {
				var me = this, offset = {}, el = this.getDom(), fixedlayer = uiUtils
						.getFixedLayer();
				function setPos(offset) {
					...
					el.style.cssText = "position:absolute;left:" + offset.left
				...
				if (e.type == "contextmenu" && hasContextmenu) {
					var menu = domUtils.getElementsByTagName(fixedlayer, "div",
							"edui-contextmenu")[0];
					if (menu) {
						setPosByCxtMenu(menu)
					} else {
						me.editor.addListener("aftershowcontextmenu", function(
								type, menu) {
							setPosByCxtMenu(menu);
						});
					}
		...
		function hideAllMenu(e) {
			var tgt = e.target || e.srcElement, cur = domUtils.findParent(tgt,
					function(node) {
						return domUtils.hasClass(node, "edui-shortcutmenu")
	...
	(function() {
		var utils = baidu.editor.utils, UIBase = baidu.editor.ui.UIBase, Breakline = baidu.editor.ui.Breakline = function(
				options) {
			this.initOptions(options);
			this.initSeparator();
		};
